...
macro(__copy_find_targets targets)	
	...
	set(VS_BIN_DIR ${CMAKE_BINARY_DIR}/src)
		...
		if(CC_BC_WIN OR CC_BC_MAC)
			if(IMPORT_LOC_DEBUG AND IMPORT_LOC_RELEASE 
				...
				add_custom_command(TARGET __auto_copy_${target} PRE_BUILD
					COMMAND ${CMAKE_COMMAND} -E make_directory "${VS_BIN_DIR}/$<$<CONFIG:Debug>:Debug>$<$<CONFIG:Release>:Release>"
					COMMAND ${CMAKE_COMMAND} -E copy_if_different  
						"$<$<CONFIG:Release>:${IMPORT_LOC_RELEASE}>"  
						"$<$<CONFIG:Debug>:${IMPORT_LOC_DEBUG}>" 
						"${VS_BIN_DIR}/$<$<CONFIG:Debug>:Debug>$<$<CONFIG:Release>:Release>"
					)
			endif()
		elseif(CC_BC_LINUX)
			if(IMPORT_LOC_RELEASE  AND EXISTS ${IMPORT_LOC_RELEASE})
				message(STATUS "copy imported target ${target}")
				add_custom_command(TARGET __auto_copy_${target} PRE_BUILD
					COMMAND ${CMAKE_COMMAND} -E make_directory "${VS_BIN_DIR}/Release/lib/"
					COMMAND ${CMAKE_COMMAND} -E copy_if_different  
						"${IMPORT_LOC_RELEASE}"  
						"${VS_BIN_DIR}/Release/lib/"
					COMMENT "auto copy ${target} [${IMPORT_LOC_RELEASE}] -> [${VS_BIN_DIR}/Release/lib/]")
			endif()
		endif()
