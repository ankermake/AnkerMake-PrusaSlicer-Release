...
// static int page = 0;
  ...
  // anker3D workaround for a bug in the USB communication controller. The semicolon character is used as an initial character
  ...
  bool anker3d_semicolon_workaround_round2 = false;
  // Buffer containing the other nibbles of semicolons to be flashed in the 2nd round.
  unsigned char anker3d_semicolon_workaround_round2_data[256];
  ...
  for (; addr < maxaddr; addr += anker3d_semicolon_workaround_round2 ? 0 : page_size) {
    ...
    if((last_addr==UINT_MAX)||(last_addr+block_size != addr)||anker3d_semicolon_workaround_round2){
      if (stk500v2_loadaddr(pgm, use_ext_addr | (addr >> addrshift)) < 0)
        return -1;
    }
    last_addr=addr;

    if (anker3d_semicolon_workaround_round2) {
        // printf("Round 2: address %d\r\n", addr);
        memcpy(buf+10, anker3d_semicolon_workaround_round2_data, block_size);
        anker3d_semicolon_workaround_round2 = false;
    } else {
        for (size_t i = 0; i < block_size; ++ i) {
            unsigned char b = m->buf[addr+i];
            if (b == ';') {
              // printf("semicolon at %d %d\r\n", addr, i);
              anker3d_semicolon_workaround_round2_data[i] = b | 0x0f0;
              b |= 0x0f;
              anker3d_semicolon_workaround_round2 = true;
            } else 
              anker3d_semicolon_workaround_round2_data[i] = 0x0ff;
            buf[i+10] = b;
        }
    }
...
{
	unsigned char buf[16];
	buf[0] = CMD_SET_UPLOAD_SIZE_ANKER3D;
	buf[1] = size & 0xff;
	buf[2] = size >> 8;
	buf[3] = size >> 16;
