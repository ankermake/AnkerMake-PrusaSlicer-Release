...
bool FirmwareDialog::priv::check_model_id()
{
	...
	// std::string line;
	// error_code ec;
	// serial.printer_write_line("ANKER Rev");
...
void FirmwareDialog::priv::avr109_wait_for_bootloader(Avr109Pid usb_pid, unsigned retries)
{
	...
	for (unsigned i = 0; i < retries && !user_cancelled; i++) {
		...
		ports.erase(std::remove_if(ports.begin(), ports.end(), [=](const SerialPortInfo &port ) {
			return port.id_vendor != USB_VID_ANKER || port.id_product != usb_pid.boot;
		}), ports.end());
...
void FirmwareDialog::priv::avr109_lookup_port(Avr109Pid usb_pid)
{
	...
	auto ports = Utils::scan_serial_ports_extended();
	ports.erase(std::remove_if(ports.begin(), ports.end(), [=](const SerialPortInfo &port ) {
		return port.id_vendor != USB_VID_ANKER ||
...
void FirmwareDialog::priv::prepare_common()
{
	std::vector<std::string> args {{
		...
		// The Anker's avrdude is patched to never send semicolons inside the data packets, as the USB to serial chip
...
void FirmwareDialog::priv::prepare_mk3()
{
	...
	std::vector<std::string> args {{
		...
		// The Anker's avrdude is patched again to never send semicolons inside the data packets.
		"-c", "arduino",
		"-P", port->port,
		"-b", "115200",
		"-D",
...
const char* FirmwareDialog::priv::avr109_dev_name(Avr109Pid usb_pid) {
	switch (usb_pid.boot) {
		case USB_PID_MMU_BOOT:
			return "Original Anker MMU 2.0 Control";
		case USB_PID_CW1_BOOT:
			return "Original Anker CW1";
		case USB_PID_CW1S_BOOT:
			return "Original Anker CW1S";
