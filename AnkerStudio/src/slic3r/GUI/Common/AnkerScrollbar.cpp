...
namespace Slic3r {
	namespace GUI {
		AnkerScrollbar::AnkerScrollbar(wxWindow* parent, wxWindowID id, wxPoint position, wxSize size, AnkerScrolledWindow* scrolledWindow, long direction, int scrollbarWidth, int tipLength) : wxPanel(parent, id, position, size)
		{
			m_parent = parent;
			...
			Bind(wxEVT_PAINT, &AnkerScrollbar::OnPaint, this);
			Bind(wxEVT_SIZE, &AnkerScrollbar::OnSize, this);
			Bind(wxEVT_ERASE_BACKGROUND, &AnkerScrollbar::OnEraseBackground, this);
			Bind(wxEVT_LEFT_DOWN, &AnkerScrollbar::OnMouseLeftDown, this);
			Bind(wxEVT_LEFT_UP, &AnkerScrollbar::OnMouseLeftUp, this);
			Bind(wxEVT_MOTION, &AnkerScrollbar::OnMouseMove, this);
			Bind(wxEVT_MOUSEWHEEL, &AnkerScrollbar::OnMouseWheel, this);
		}

		void AnkerScrollbar::SetViewStart(int start)
		{
			m_viewStartInScrollUnits = start;
			m_viewStartInPixels = start;
		}

		void AnkerScrollbar::SetTipColor(wxColour color)
		{
			m_tipColor = color;
		}

		void AnkerScrollbar::SetMarginColor(wxColour color)
		{
			m_marginColor = color;
		}

		void AnkerScrollbar::SetScrollbarColor(wxColour color)
		{
			m_scrollbarColor = color;
		}

		void AnkerScrollbar::SetScrollbarTip(int len)
		{
			m_tipLength = len;
		}

		void AnkerScrollbar::SetVirtualDim(int pixelsPerUnit, int noUnits)
		{
		...
		void AnkerScrollbar::OnSize(wxSizeEvent& WXUNUSED(event))
		{
			if (m_direction == wxVSCROLL)
			{
				m_actualDim = GetSize().GetHeight();
		...
		void AnkerScrollbar::OnPaint(wxPaintEvent& event)
		...
		void AnkerScrollbar::OnEraseBackground(wxEraseEvent& event)
		{
			// necessary to avoid automatic background erasing
			SetBackgroundColour(wxColor(255, 255, 255));
		}

		void AnkerScrollbar::OnMouseLeftDown(wxMouseEvent& event)
		{
		...
		void AnkerScrollbar::OnMouseLeftUp(wxMouseEvent& event)
		{
			if (HasCapture())
			{
				ReleaseMouse();
		...
		void AnkerScrollbar::OnMouseMove(wxMouseEvent& event)
		{
		...
		void AnkerScrollbar::OnMouseWheel(wxMouseEvent& event)
		{
			int dMotion, actualDim;
