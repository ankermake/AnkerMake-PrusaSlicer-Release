...
#include "AnkerScrolledWindow.hpp"
#include "AnkerScrollbar.hpp"
namespace Slic3r {
    namespace GUI {
        AnkerScrolledWindow::AnkerScrolledWindow(wxWindow* parent, wxWindowID id, wxPoint position, wxSize size, long style, int marginWidth, int scrollbarWidth, int tipLength)
            : wxScrolled<wxWindow>(parent, id, position, size, style)
        {
            ...
            if (bVertical) {
                ...
                m_rightScrollbar = new AnkerScrollbar(m_scroll_win, -1, wxPoint(0, 0), wxSize(scrollbarWidth, size.GetHeight()), this, wxVSCROLL, scrollbarWidth, tipLength);
            }
            else if (bHorizontal) {
                ...
                m_bottomScrollbar = new AnkerScrollbar(m_horizontalSplitter, -1, wxPoint(0, 0), wxSize(size.GetWidth() - marginWidth, marginWidth), this, wxHSCROLL, scrollbarWidth,
                    tipLength);
            ...
            Bind(wxEVT_SIZE, &AnkerScrolledWindow::OnSize, this);
            Bind(wxEVT_SCROLLWIN_TOP, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_BOTTOM, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_LINEUP, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_LINEDOWN, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_PAGEUP, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_PAGEDOWN, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_THUMBTRACK, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_SCROLLWIN_THUMBRELEASE, &AnkerScrolledWindow::OnScroll, this);
            Bind(wxEVT_MOUSEWHEEL, &AnkerScrolledWindow::OnMouseWheel, this);
        }

        void AnkerScrolledWindow::OnMouseWheel(wxMouseEvent& event)
        {
        ...
        void AnkerScrolledWindow::SetBackgroundColour(wxColour color)
        {
            wxWindow::SetBackgroundColour(color);
            if (m_verticalSplitter) m_verticalSplitter->SetBackgroundColour(color);

            m_userPanel->SetBackgroundColour(color);
            if(m_scroll_win) m_scroll_win->SetBackgroundColour(color);
        }

        void AnkerScrolledWindow::SetMarginColor(wxColour color)
        {
            if (m_rightScrollbar) m_rightScrollbar->SetMarginColor(color);
            if (m_bottomScrollbar) m_bottomScrollbar->SetMarginColor(color);
        }

        void AnkerScrolledWindow::SetScrollbarColor(wxColour color)
        {
        ...
        void AnkerScrolledWindow::SetScrollbarTip(int len)
        {
            if (m_rightScrollbar) m_rightScrollbar->SetScrollbarTip(len);
            if (m_bottomScrollbar) m_bottomScrollbar->SetScrollbarTip(len);
        }

        void AnkerScrolledWindow::SetVirtualSize(int x, int y) { SetScrollbars(1, 1, x, y); }

        void AnkerScrolledWindow::SetVirtualSize(wxSize& size) { SetScrollbars(1, 1, size.GetWidth(), size.GetHeight()); }

        void AnkerScrolledWindow::SetScrollbars(int pixelsPerUnitX, int pixelsPerUnitY, int noUnitsX, int noUnitsY, int xPos, int yPos, bool noRefresh)
        {
        ...
        void AnkerScrolledWindow::OnSize(wxSizeEvent& event)
        {
        ...
        void AnkerScrolledWindow::OnScroll(wxScrollWinEvent& event)
        {
            int startX, startY;
            GetViewStart(&startX, &startY);
            if (m_rightScrollbar) {
