...
wxMenu* MenuFactory::append_submenu_add_generic(wxMenu* menu, ModelVolumeType type)
{
    ...
    //        [type](wxCommandEvent&) { objectbar()->load_subobject(type, true); }, "", menu);
    //}
...
wxMenuItem* MenuFactory::append_menu_item_simplify(wxMenu* menu)
{
    wxMenuItem* menu_item = append_menu_item(menu, wxID_ANY, _L("Simplify model"), "",
        [](wxCommandEvent&) { 
            ...
            Slic3r::GUI::GLGizmosManager& gizmos_mgr = plater->canvas3D()->get_gizmos_manager();

            // Do not simplify when a gizmo is open. There might be issues with updates
            // and what is worse, the snapshot time would refer to the internal stack.
            if (!gizmos_mgr.check_gizmos_closed_except(Slic3r::GUI::GLGizmosManager::EType::Simplify))
                return;

            if (gizmos_mgr.get_current_type() == Slic3r::GUI::GLGizmosManager::Simplify) {
                // close first
                gizmos_mgr.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Simplify);
            }
            gizmos_mgr.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Simplify);
        }, "", menu,
