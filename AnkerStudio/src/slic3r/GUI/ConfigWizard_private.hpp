...
struct Bundle
{
	...
	bool is_anker_bundle{ false };

	Bundle() = default;
	Bundle(Bundle&& other);

	// Returns false if not loaded. Reason for that is logged as boost::log error.
	bool load(fs::path source_path, BundleLocation location, bool is_anker_bundle = false);
...
struct BundleMap : std::map<std::string /* = vendor ID */, Bundle>
{
	...
	Bundle& anker_bundle();
	const Bundle& anker_bundle() const;
};
...
struct PageWelcome: ConfigWizardPage
{
    ...
    bool reset_user_profile() const { return !PresetBundle::BoolAnkerMain && cbox_reset != nullptr ? cbox_reset->GetValue() : false; }
