...
static const Slic3r::ColorRGBA DEFAULT_BG_DARK_COLOR  = { 0.094f, 0.098f, 0.106f, 1.0f };
static const Slic3r::ColorRGBA DEFAULT_BG_LIGHT_COLOR = { 0.094f, 0.098f, 0.106f, 1.0f };
...
void GLCanvas3D::LayersEditing::render_layer_height_change(GLCanvas3D& canvas)
{
    int model_idx = canvas.m_selection.get_object_idx();
    if (model_idx > -1 && m_model_object)
    {
        double layer_height = m_profile.old_layer_height;
        layer_height = const_cast<ModelObject*>(m_model_object)->config.option("layer_height")->getFloat();
        if (m_profile.old_layer_height != layer_height) {
            DynamicPrintConfig config = *canvas.m_config;
            config.set_key_value("layer_height", new ConfigOptionFloat(layer_height));
            set_config(&config);
            m_profile.old_layer_height = layer_height;
            reset_layer_height_profile(canvas);
            render_volumes(canvas, canvas.m_volumes);
            update_slicing_parameters();
            set_config(canvas.m_config);
        }
    }
}
...
bool GLCanvas3D::init()
{
    ...
    // Anker: do not render UI false
    m_gizmos.set_UI_rendered(false);
    ...
    post_event(SimpleEvent(EVT_GLCANVAS_INITIALIZED));
    ...
    for (GLVolume* vol : m_volumes.volumes) {
      if ((mo == nullptr || m_model->objects[vol->composite_id.object_id] == mo)
            ...
            if (it != raycasters->end())
                if (get_gizmos_manager().get_current_type() == GLGizmosManager::MmuSegmentation) {
                    (*it)->set_active(true);
                }
                else {
...
BoundingBoxf3 GLCanvas3D::volumes_bounding_box() const
{
    ...
    for (const GLVolume* volume : m_volumes.volumes) {
        if ((volume != nullptr && !m_apply_zoom_to_volumes_filter) || ((volume != nullptr) && volume->zoom_to_volumes))
...
void GLCanvas3D::zoom_to_bed()
{
    ...
    box.min.z() = 0.0;
    box.max.z() = 0.0;
    auto delta = (box.max - box.min) * 0.25;
    box.max += delta;
    box.min -= delta;
    _zoom_to_box(box);
}
...
//Anker: offer_render pic
void GLCanvas3D::render_akpic(post_gcode::picData& akpic_single, unsigned int w, unsigned int h, double offset_y, double offset_z, const std::array<unsigned int, 2>& layers_z_range)
{
    switch (OpenGLManager::get_framebuffers_type())
    {
    case OpenGLManager::EFramebufferType::Arb: { _render_akpic_framebuffer(akpic_single, w, h, offset_y, offset_z, layers_z_range); break; }
    case OpenGLManager::EFramebufferType::Ext: { _render_akpic_framebuffer_ext(akpic_single, w, h, offset_y, offset_z, layers_z_range); break; }
    default: { _render_akpic_legacy(akpic_single, w, h, offset_y, offset_z, layers_z_range); break; }
    }
}
...
void GLCanvas3D::select_all()
{
    ...
    reset_main_toolbar_toggled_state();
...
void GLCanvas3D::set_selected_visible(bool visible)
{
    for (unsigned int i : m_selection.get_volume_idxs()) {
        ...
        volume->color.set(3, visible ? 1.f : GLVolume::MODEL_HIDDEN_COL[3]);
        volume->render_color.set(3, volume->color.a());
...
void GLCanvas3D::set_gcode_window_visibility(bool visible)
{ 
    m_gcode_viewer.set_gcode_window_visibility(visible);
    m_dirty = true; 
    request_extra_frame();
}

bool GLCanvas3D::get_gcode_window_visibility()
{
    return m_gcode_viewer.get_gcode_window_visibility();
}
...
void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh)
{
    ...
    if (m_volumes.volumes != glvolumes_new) {
        ...
        //no need to update if extra volume is wipe tower
        if (m_volumes.volumes.size() == glvolumes_new.size() + 1) {
            for (auto& volume : m_volumes.volumes) {
                // some times volume is null from preview back to view3d when slicing 
                if (volume && volume->is_wipe_tower) {
    ...
    if (printer_technology == ptFFF && m_config->has("nozzle_diameter") && (m_canvas_type != ECanvasType::CanvasAssembleView)) {
    ...
    if (!m_volumes.empty() && m_canvas_type != ECanvasType::CanvasAssembleView) {
    ...
    if (m_selection.is_empty()) {
        ...
        auto manip = wxGetApp().aobj_manipul();
        if (manip != nullptr)
            manip->set_dirty();
    }
            ...
            if (!m_moving) {
                if ((any_gizmo_active || evt.CmdDown() || m_hover_volume_idxs.empty()) && m_mouse.is_start_position_3D_defined()) {
                    ...
                    if (wxGetApp().app_config->get_bool("use_free_camera"))
                        ...
                        // See GH issue #3816.
                        // TODO assemble view do not limit rotate in assemble view
                        Camera& camera = wxGetApp().plater()->get_camera();
...
{
    m_tooltip_enabled = false;
    ANKER_LOG_DEBUG << "_refresh_if_shown_on_screen start";
    _refresh_if_shown_on_screen();
    ANKER_LOG_DEBUG << "_refresh_if_shown_on_screen end";
    m_tooltip_enabled = true;
}

Size GLCanvas3D::get_canvas_size_with_no_scale() const
{
    int w = 0;
    int h = 0;
...
bool GLCanvas3D::can_gizmos()
{
    return !wxGetApp().plater()->model().objects.empty() && !get_selection().is_empty() && m_gizmos.is_enabled();
}
...
void GLCanvas3D::handle_layers_data_focus_event(const t_layer_height_range range, const AnkerEditorType type)
{
    std::string field = "layer_" + std::to_string(type) + "_" + float_to_string_decimal_point(range.first) + "_" + float_to_string_decimal_point(range.second);
    handle_sidebar_focus_event(field, true);
}
...
{
    return (m_canvas != nullptr) ? m_canvas->IsShownOnScreen() || m_force_on_screen : false;
}
...
bool GLCanvas3D::_render_search_list(float pos_x)
{
    ...
    AnkerSidebarNew& sidebarnew = wxGetApp().sidebarnew();
    std::string& search_line = sidebarnew.get_search_line();
    ...
    imgui->search_list(ImVec2(45 * em, 30 * em), &search_string_getter, s,
       /* sidebar.get_searcher().view_params,*/
        sidebarnew.get_searcher().view_params,
    ...
    if (selected >= 0) {
        ...
        if (selected != 9999) {
            ...
            //sidebar.jump_to_option(selected);
            sidebarnew.jump_to_option(selected);
            return true;
        }
        action_taken = true;
    }
...
static void debug_output_thumbnail(const ThumbnailData& thumbnail_data)
{
    ...
    image.SaveFile("C:/anker/test/test.png", wxBITMAP_TYPE_PNG);
}
...
void GLCanvas3D::_render_thumbnail_internal(ThumbnailData& thumbnail_data, const ThumbnailsParams& thumbnail_params, const GLVolumeCollection& volumes, Camera::EType camera_type)
{
    ...
    for (GLVolume* vol : visible_volumes) {
        ...
        vol->is_active = true;
        if (vol->mmu_mesh != nullptr && get_filament_colors().size() > 1) {
            vol->set_render_color(vol->color);
            vol->render_mmu(get_filament_colors(), camera);
        }
        else {
...
//Anker: offer_render pic
void GLCanvas3D::_render_akpic_internal( double offset_y, double offset_z, const std::array<unsigned int, 2>& layers_z_range)
{
    //TODO w,h 
    glsafe(::glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT));
    m_gcode_viewer.set_layers_z_range_inter(layers_z_range);
    //m_gcode_viewer.set_layers_z_range(layers_z_range);
    glsafe(::glViewport(0, 0, (GLsizei)2048, (GLsizei)1196));
    m_gcode_viewer.render_toolpaths_ext(offset_y, offset_z);
}

void GLCanvas3D::_render_akpic_framebuffer(post_gcode::picData& akpic_single, unsigned int w, unsigned int h, double offset_y, double offset_z, const std::array<unsigned int, 2>& layers_z_range)
{
    akpic_single.set(w, h);
    if (!akpic_single.is_valid())
        return;
    ...
    if (::glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
        _render_akpic_internal(offset_y, offset_z, layers_z_range);
        ...
        if (multisample) {
            ...
            if (::glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
                ...
                glsafe(::glBindFramebuffer(GL_READ_FRAMEBUFFER, resolve_fbo));
                glsafe(::glReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, (void*)akpic_single.pixels.data()));
            }
...
void GLCanvas3D::_render_akpic_framebuffer_ext(post_gcode::picData& akpic_single, unsigned int w, unsigned int h, double offset_y, double offset_z, const std::array<unsigned int, 2>& layers_z_range)
{
    akpic_single.set(w, h);
    if (!akpic_single.is_valid())
        return;
...
void GLCanvas3D::_render_akpic_legacy(post_gcode::picData& akpic_single, unsigned int w, unsigned int h, double offset_y, double offset_z, const std::array<unsigned int, 2>& layers_z_range)
{
    // check that thumbnail size does not exceed the default framebuffer size
    const Size& cnv_size = get_canvas_size();
    unsigned int cnv_w = (unsigned int)cnv_size.get_width();
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    if (!m_main_toolbar.init(ColorRGBA(0.161f, 0.165f, 0.176f, 1.0f))) {
...
//    m_main_toolbar.set_layout_type(GLToolbar::Layout::Vertical);
    ...
    m_main_toolbar.set_border_outer(20.0f);
    m_main_toolbar.set_separator_size(5);
    m_main_toolbar.set_gap_size(0);
    m_main_toolbar.set_scale(1.2f);

    GLToolbarItem::Data item;
    item.name = "add";
    item.icon_filename = "add_model.svg";
    ...
    item.left.action_callback = [this]() { 
        ANKER_LOG_INFO << "Main Tool: Arrange";
        if (m_canvas != nullptr) 
    ...
    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Move)->get_name(false, false);
    item.icon_filename = "move.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Move)->get_name();
    item.sprite_id = 2;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Move); };
    item.enabling_callback = [this]()->bool { return can_gizmos(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Move)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Rotate)->get_name(false, false);
    item.icon_filename = "rotate.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Rotate)->get_name();
    item.sprite_id = 3;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Rotate); };
    item.enabling_callback = [this]()->bool { return can_gizmos(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Rotate)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Scale)->get_name(false, false);
    item.icon_filename = "scale.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Scale)->get_name();
    item.sprite_id = 4;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Scale); };
    item.enabling_callback = [this]()->bool { return can_gizmos(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Scale)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Flatten)->get_name(false, false);
    item.icon_filename = "place.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Flatten)->get_name();
    item.sprite_id = 5;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Flatten); };
    item.enabling_callback = [this]()->bool { return can_gizmos() && get_selection().is_single_full_instance(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Flatten)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;
	...
	item.sprite_id = 6;
	item.left.action_callback = [this]() { 
        ANKER_LOG_INFO << "Main Tool: Split Objects";
        if (m_canvas != nullptr) 
	...
	item.sprite_id = 7;
	item.left.action_callback = [this]() {
        ANKER_LOG_INFO << "Main Tool: Split Parts";
        if (m_canvas != nullptr) 
    ...
    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Cut)->get_name(false, false);
    item.icon_filename = "cut.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Cut)->get_name();
    item.sprite_id = 8;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Cut); };
    item.enabling_callback = [this]()->bool { return can_gizmos() && get_selection().is_single_full_instance(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Cut)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::MmuSegmentation)->get_name(false, false);
    item.icon_filename = "mmu_segmentation.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::MmuSegmentation)->get_name();
    item.sprite_id = 9;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::MmuSegmentation); };
    item.enabling_callback = [this]()->bool {
        Selection& selection = get_selection();
        if (selection.is_empty())
            return false;

        Selection::IndicesList volIdxs = selection.get_volume_idxs();
        for (auto itr = volIdxs.begin(); itr != volIdxs.end(); itr++)
            if (selection.get_volume(*itr)->is_outside)
                return false;

        return can_gizmos() && get_selection().is_single_full_instance();
    };
    item.visibility_callback = [this]()->bool { return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::MmuSegmentation)->is_selectable(); };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::MmuSegmentation)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Seam)->get_name(false, false);
    item.icon_filename = "seam.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Seam)->get_name();
    item.sprite_id = 10;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Seam); };
    item.enabling_callback = [this]()->bool { 
        Selection& selection = get_selection();
        if (selection.is_empty())
            return false;
    ...
    //item.visibility_callback = [this]()->bool { return m_main_toolbar_advanced; };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Seam)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;

    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::FdmSupports)->get_name(false, false);
	item.icon_filename = "supports.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::FdmSupports)->get_name();
	item.sprite_id = 11;
    item.left.toggable = true;
	item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::FdmSupports); };
    item.enabling_callback = [this]()->bool {
        Selection& selection = get_selection();
        if (selection.is_empty())
            return false;
    ...
    //item.visibility_callback = [this]()->bool { return m_main_toolbar_advanced; };
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::FdmSupports)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
	if (!m_main_toolbar.add_item(item))
		return false;

    item = GLToolbarItem::Data();
    item.name = "Auto bed";
    item.icon_filename = "auto_orient.svg";
    item.tooltip = _u8L("engin_auto_orient");
    item.sprite_id = 12;
    item.left.toggable = true;
    item.left.action_callback = [this]() {
        ANKER_LOG_INFO << "Main Tool: auto bed";
       // m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::AutoOrient);
        if (m_canvas != nullptr)
            wxPostEvent(m_canvas, SimpleEvent(EVT_GLTOOLBAR_AUTO_BED)); };
    item.enabling_callback = []()->bool { return wxGetApp().plater()->can_auto_bed(); };
    if (!m_main_toolbar.add_item(item))
        return false;

    // Add by virgil.
    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Measure)->get_name(false, false);
    item.icon_filename = "measure.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Measure)->get_name();
    item.sprite_id = 13;
    item.left.toggable = true;
    item.left.action_callback = [this]() { m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Measure); };
    item.enabling_callback = [this]()->bool {
        Selection& selection = get_selection();
        if (selection.is_empty())
            return false;
    ...
    item.toggled_callback = [this]()->bool {return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Measure)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On; };
    if (!m_main_toolbar.add_item(item))
        return false;


    item = GLToolbarItem::Data();
    item.name = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Emboss)->get_name(false, false);
    item.icon_filename = "toolbar_text.svg";
    item.tooltip = m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Emboss)->get_name();
    item.sprite_id = 14;
    item.left.toggable = true;
    item.left.action_callback = [this]() {
        ANKER_LOG_INFO << "Main Tool: Emboss";
        Plater* p = wxGetApp().plater();
        if (p != nullptr) {
            const Selection& sel = p->get_selection();
            if (sel.volumes_count() == 1) {
                const GLVolume* gl_vol = sel.get_first_volume();
                const ModelVolume* vol = p->model().objects[gl_vol->object_idx()]->volumes[gl_vol->volume_idx()];
                if (vol->text_configuration.has_value()) {
                    ...
                    m_gizmos.reset_all_states();
                    m_gizmos.open_gizmo(Slic3r::GUI::GLGizmosManager::EType::Emboss);
                    return;
                }
            }
        }

        GLGizmosManager::EType gizmo_type = GLGizmosManager::EType::Emboss;
        ...
        if (gizmo_type == GLGizmosManager::Emboss) {
            ...
            bool initEmboss = false;
            if (m_main_toolbar.is_item_pressed("layersediting")) {
                force_main_toolbar_left_action(get_main_toolbar_item_id("layersediting"));
            }
            if (screen_position.has_value()) {
                initEmboss = emboss->create_volume(volume_type, *screen_position);
            }
            else {
                initEmboss = emboss->create_volume(volume_type);
            }
            if (!initEmboss) return;
        }
    };
    item.enabling_callback = [this]()->bool {
        return m_gizmos.is_enabled();
    };
    item.toggled_callback = [this]()->bool {
        return m_gizmos.get_gizmo(Slic3r::GUI::GLGizmosManager::EType::Emboss)->get_state() == Slic3r::GUI::GLGizmoBase::EState::On;
    };
    if (!m_main_toolbar.add_item(item))
        return false;
    ...
    item.icon_filename = "layer_editing.svg";
    ...
    item.toggled_callback = [this]()->bool { return m_layers_editing.is_enabled(); };
    if (!m_main_toolbar.add_item(item))
        return false;
    
    item = GLToolbarItem::Data();
    item.name = "assembly_view";
    item.icon_filename = "assemble_view.svg";
...
bool GLCanvas3D::_init_undoredo_toolbar()
{
    ...
    if (!m_undoredo_toolbar.init(ColorRGBA(0.161f, 0.165f, 0.176f, 1.0f))) {
        // unable to init the toolbar texture, disable it
...
//    m_undoredo_toolbar.set_layout_type(GLToolbar::Layout::Vertical);
    ...
    m_undoredo_toolbar.set_border(2.0f);
    m_undoredo_toolbar.set_border_outer(20.0f);
    m_undoredo_toolbar.set_separator_size(5);
    m_undoredo_toolbar.set_gap_size(0);
    m_undoredo_toolbar.set_scale(1.2f);
...
void GLCanvas3D::_refresh_if_shown_on_screen()
{
    if (_is_shown_on_screen()) {
        ...
        ANKER_LOG_DEBUG << "render start";
        render();
        ANKER_LOG_DEBUG << "render end";
    ...
    if (hit.is_valid()) {
        ...
        case SceneRaycaster::EType::Volume:
        {
            if (0 <= hit.raycaster_id && hit.raycaster_id < (int)m_volumes.volumes.size()) {
                ...
                if ((volume->is_active || get_gizmos_manager().get_current_type() == GLGizmosManager::MmuSegmentation) 
                    && !volume->disabled && (volume->composite_id.volume_id >= 0 || m_render_sla_auxiliaries)) {
...
void GLCanvas3D::_render_objects(GLVolumeCollection::ERenderType type)
{
    ...
    if (m_canvas_type == CanvasAssembleView)
        ...
        m_volumes.set_show_sinking_contours(!m_gizmos.is_hiding_instances() && m_gizmos.get_current_type() != GLGizmosManager::Simplify);

    GLShaderProgram* shader = wxGetApp().get_shader("gouraud");
    if (shader != nullptr) {
        shader->start_using();

        shader->set_uniform("use_assemble_view", m_canvas_type == ECanvasType::CanvasAssembleView);
        switch (type)
        {
        default:
...
//                GLGizmosManager::EType type = gm.get_current_type();
        ...
        case GLVolumeCollection::ERenderType::Transparent:
        {
            ...
            m_volumes.render(type, false, wxGetApp().plater()->get_camera().get_view_matrix(), wxGetApp().plater()->get_camera().get_projection_matrix(), [this, type](const GLVolume& volume) {
                if (m_canvas_type == ECanvasType::CanvasAssembleView) { return !volume.is_modifier; }
                else { return true; }
            });
...
void GLCanvas3D::_check_and_update_toolbar_icon_scale()
{
    ...
    int vItems_cnt = m_gizmos.get_selectable_icons_cnt() + 3; // +3 means a place for top and view toolbars and separators in gizmos toolbar

    // calculate scale needed for items in the gizmos toolbar
    const float new_v_scale = cnv_size.get_height() / (vItems_cnt * GLGizmosManager::Default_Icons_Size);

    // set minimum scale as a auto scale for the toolbars
    float new_scale = /*std::min(new_h_scale, new_v_scale)*/new_h_scale;
#if ENABLE_RETINA_GL
    new_scale /= m_retina_helper->get_scale_factor();
#endif

    int barWidth = AnkerLength(150);
    if ((cnv_size.get_width() - items_cnt * GLGizmosManager::Default_Icons_Size * new_scale) < barWidth)
    {
        new_scale = (cnv_size.get_width() - barWidth) / items_cnt / GLGizmosManager::Default_Icons_Size;
    }
...
void GLCanvas3D::_render_undoredo_toolbar()
{
    ...
    GLToolbar& collapse_toolbar = wxGetApp().plater()->get_collapse_toolbar();
    const float collapse_toolbar_width = collapse_toolbar.is_enabled() ? collapse_toolbar.get_width() : 0.0f;
    const float left = m_main_toolbar.get_width() - 0.5f * (m_main_toolbar.get_width() + m_undoredo_toolbar.get_width() + collapse_toolbar_width) + 0.1f * m_undoredo_toolbar.get_width();
...
void GLCanvas3D::_render_return_toolbar() const
{
    ...
    float font_size = ImGui::GetFontSize();
#ifdef __WINDOWS__
    ImVec2 real_size = ImVec2(font_size * 5, font_size * 2);
#else
    ...
    //ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.38f, 0.82f, 0.38f, 0.5f));
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.161f, 0.165f, 0.176f, 1.0f));
    ...
    if (ImGui::ImageTextButton(real_size, _u8L(L("Return")).c_str(), m_return_toolbar.get_return_texture_id(), button_icon_size, uv0, uv1, -1, bg_col, tint_col, margin)) {
        if (m_canvas != nullptr)
            wxPostEvent(m_canvas, SimpleEvent(EVT_GLVIEWTOOLBAR_3D));
        const_cast<GLGizmosManager*>(&m_gizmos)->reset_all_states();
        wxGetApp().plater()->canvas3D()->get_gizmos_manager().reset_all_states();
    }
    ImGui::PopStyleColor(5);
    ImGui::PopStyleVar(1);
...
{
    // TODO Add Assemble View color tool bar
}
...
void GLCanvas3D::_render_assemble_control()
{
    ...
    const float value_size = imgui->calc_text_size(_L("3.00")).x + text_padding * 2;
    ...
    //check ratio changed
    if (m_explosion_ratio != GLVolume::explosion_ratio) {
        m_explosion_ratio = std::clamp(m_explosion_ratio, 1.0f, 3.0f);
        for (GLVolume* volume : m_volumes.volumes) {
            volume->set_bounding_boxes_as_dirty();
        }
        GLVolume::explosion_ratio = m_explosion_ratio;
        post_event(SimpleEvent(EVT_EXPLOSION_VALUE_CHANGE));
    }
}
...
void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
{
    ...
    case EWarning::ToolpathOutside:    text = _u8L("common_slicepopup_outside"); error = ErrorType::SLICING_ERROR; break;
    ...
    case EWarning::ObjectClashed:
        text = _u8L("common_slicepopup_objectoutside");
    ...
    case EWarning::GCodeConflict: {
        ...
        text = format(_u8L("common_print_slice_wipetower_further_hint"/*"Conflicts in G-code paths have been detected at layer %1%, z=%2$.2f mm. Please reposition the conflicting objects (%3% <-> %4%) further apart."*/),
            ...
            auto     action_fn = [id, layer_id](wxEvtHandler*) {
                ...
                if (iter != objects.end()) {
                    ...
                    wxGetApp().CallAfter([obj_idx, layer_id]() {
                        ...
                        wxGetApp().plater()->select_view_3D(VIEW_MODE_3D);
...
int GLCanvas3D::get_object_idx(const ModelObject* mo)
{
    for (int object_idx = 0; object_idx < (int)m_model->objects.size(); ++object_idx) {
        const ModelObject* model_object = m_model->objects[object_idx];
        if (mo == model_object) {
