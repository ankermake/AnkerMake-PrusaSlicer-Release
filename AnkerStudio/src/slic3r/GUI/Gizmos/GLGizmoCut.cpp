...
std::string PositionZLineEditInitValue = "11.75";
const size_t ctrlHeight = 20;
...
namespace Slic3r {
	namespace GUI {
		...
		bool GLGizmoCut3D::render_cut_mode_combo()
		{
			...
			if (is_changed) {
				if (CutMode::cutTongueAndGroove == CutMode(m_mode)) {
					m_cut_to_parts_show = true;
				}
				else if (CutMode::cutPlanar == CutMode(m_mode)) {
					m_cut_to_parts_show = false;
...
// slider_with + item_in_gap + first_input_width + item_out_gap + slider_with + item_in_gap + second_input_width
			...
			tolerance_in = tolerance * float(m_imperial_units ? ObjectManipulation::in_to_mm : 1.0);
...
#ifdef ANKER_SIDEBAR_UI
			ANKER_LOG_INFO << "GLGizmoCut3D::set_input_window_state: " << on;
			if (on) {
				initGUI();
			}
			else {
		...
		{
			return /*wxGetApp().get_mode() != comSimple*/true;
		}
		...
		void GLGizmoCut3D::render_connectors_input_window(CutConnectors& connectors, float x, float y, float bottom_limit)
		{
			...
			if (m_connector_type == CutConnectorType::Snap) {
				render_snap_specific_input(_u8L("Bulge"), _L("Bulge proportion related to radius"), m_snap_bulge_proportion, 0.15f, 5.f, 100.f * m_snap_space_proportion);
				render_snap_specific_input(_u8L("engin_cut_space"), _L("Space proportion related to radius"), m_snap_space_proportion, 0.3f, 10.f, 50.f);
			}
			...
			float f_scale = 1.0/*m_parent.get_gizmos_manager().get_layout_scale()*/;
...
#ifdef ANKER_SIDEBAR_UI
			if (m_buildVolumeTextSize) {
				m_buildVolumeTextSize->SetLabelText(size);
			}
#endif
		}
...
// slider_with + item_in_gap + input_width
		...
		void GLGizmoCut3D::render_cut_plane_input_window(CutConnectors& connectors, float x, float y, float bottom_limit)
		{
			...
			if (mode == CutMode::cutPlanar || mode == CutMode::cutTongueAndGroove) {
				...
				if (mode == CutMode::cutPlanar) {
					...
					m_imgui->disabled_begin(is_cut_plane_init && !has_connectors);
					wxString reset_act_name = _L("Reset cut");
					wxString reset_cut_tip = _L("Reset cutting plane and remove connectors");
					if (m_imgui->button(reset_act_name, reset_cut_tip)) {
						Plater::TakeSnapshot snapshot(wxGetApp().plater(), reset_act_name, UndoRedo::SnapshotType::GizmoAction);
						reset_cut_plane();
						reset_connectors();
					}
					m_imgui->disabled_end();
		...
		void GLGizmoCut3D::updateAnkerData()
		{
			CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
			init_input_window_data(connectors);
#ifdef ANKER_SIDEBAR_UI
			if (m_connectors_editing) // connectors mode
			{
				bool disable = m_connectors_editing && connectors.empty();
				if (m_flipCutPlaneBtn && m_connectorsResetBtn && m_flipCutPlaneBtnEnable != !disable) {
					m_flipCutPlaneBtn->Enable(!disable);
					m_connectorsResetBtn->Enable(!disable);
					m_connectorsPanel->Refresh();
					m_flipCutPlaneBtnEnable = !disable;
				}

			}
			else {
				if (m_cutPositionZLineEdit) {
					double value = m_plane_center[Z];
					double lastValue = 0.0;
					m_cutPositionZLineEdit->GetValue().ToDouble(&lastValue);
					if (abs(lastValue - value) >= 0.01) {
						if (m_cutPositionZEditMode == AnkerLineEditUnitMode_None) {
							m_cutPositionZLineEdit->SetValue(wxString::Format(wxT("%.2f"), value));
							m_cutPositionResetBtn->Enable(m_bb_center != m_plane_center);
							m_cutPanel->Refresh();
						}
						else if (m_cutPositionZEditMode == AnkerLineEditUnitMode_Edited) {
							m_cutPositionZEditMode = AnkerLineEditUnitMode_None;
							m_plane_center[2] = lastValue;
							m_cutPositionResetBtn->Enable(m_bb_center != m_plane_center);
						m_cutPanel->Refresh();
						}
				...
				if (mode == CutMode::cutPlanar || mode == CutMode::cutTongueAndGroove) {
					...
					const bool is_cut_plane_init = m_rotation_m.isApprox(Transform3d::Identity()) && m_bb_center.isApprox(m_plane_center);
					if (m_resetCutBtn) {
						bool tmpEnable = !(is_cut_plane_init && !has_connectors);
						if (tmpEnable != m_resetCutBtnEnable) {
							m_resetCutBtn->Enable(tmpEnable);
							m_resetCutBtn->Refresh();
							m_resetCutBtnEnable = tmpEnable;
						}
					}
				}
			}
			...
			wxString out = "";
			if (!m_invalid_connectors_idxs.empty()) {
				out = _L("Invalid connectors detected") + wxString(" : -1 ") + _L("connector is out of \nobject");
			}
			if (m_lastInvalidConnectorsInfoStr != out) {
				if (m_invalidConnectorsInfoText && m_warningInvalidConnectorsImage) {
					m_invalidConnectorsInfoText->SetLabel(out);
					m_warningInvalidConnectorsImage->visible = !out.empty();
					m_performCutBtn->Enable(out.empty());
					m_lastInvalidConnectorsInfoStr = out;
					container->Refresh();
				}
			}
			...
			wxString cutInfo = "";
			if (!m_keep_upper && !m_keep_lower)
				cutInfo = _L("Select at least one object to keep after cutting.");
			if (!has_valid_contour())
				cutInfo = _L("Cut plane is placed out of object");
			else if (!has_valid_groove())
				cutInfo = _L("Cut plane with groove is invalid");

			if (m_lastInvalidCutInfoStr != cutInfo) {
				if (m_invalidCutInfoText && m_warningInvalidCutInfoImage) {
					m_invalidCutInfoText->SetLabel(cutInfo);
					m_warningInvalidCutInfoImage->visible = !cutInfo.empty();
					m_performCutBtn->Enable(cutInfo.empty());
					m_lastInvalidCutInfoStr = cutInfo;
					container->Refresh();
				}
			}
#endif
		...
		{
#ifdef ORCA_UI
			auto& shortcuts = /*m_connectors_editing ? m_shortcuts_connector :*/ m_shortcuts;
		...
		static void check_objects_after_cut(const ModelObjectPtrs& objects)
		{
			...
			if (is_windows10() && !err_objects_idxs.empty()) {
				...
				if (dlg.ShowModal() == wxID_YES) {
					...
					// Show info dialog
					wxString msg = MenuFactory::get_repair_result_message(succes_models, failed_models);
		...
		bool GLGizmoCut3D::add_connector(CutConnectors& connectors, const Vec2d& mouse_position)
		{
			...
			if (unproject_on_cut_plane(mouse_position.cast<double>(), pos, pos_world)) {
				const ModelObject* mo = m_c->selection_info()->model_object();
				if (mo && mo->bounding_box_exact().contains(pos_world)) {
...
#ifdef ANKER_SIDEBAR_UI
		void GLGizmoCut3D::initGUI()
		{
			wxGetApp().plater()->sidebarnew().setMainSizer();
			if (m_pInputWindowSizer == nullptr)
			{
				m_pInputWindowSizer = new wxBoxSizer(wxVERTICAL);
				container = new AnkerTitledPanel(&(wxGetApp().plater()->sidebarnew()), 32, 0, wxColour(PANEL_TITLE_BACK_DARK_RGB_INT));
				...
				m_cutPanel = new wxPanel(container);
				m_cutPanel->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
				wxBoxSizer* cutPanelSizer = new wxBoxSizer(wxVERTICAL);
				m_cutPanel->SetSizer(cutPanelSizer);

				container->setContentPanel(m_cutPanel);

				// Mode
				{
					{
						wxBoxSizer* modeSizer = new wxBoxSizer(wxHORIZONTAL);
						cutPanelSizer->Add(modeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_modeText = new wxStaticText(m_cutPanel, wxID_ANY, _L("Mode")/*engin_cut_mode*/);
						m_modeText->SetFont(ANKER_FONT_NO_1);
						m_modeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						modeSizer->Add(m_modeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						modeSizer->AddSpacer(10);

						wxArrayString itemList;
						itemList.Add(_L("Planar")/*engin_cut_planar*/);
						itemList.Add(_L("Dovetail")/*engin_cut_dovetail*/);

						m_modeComboBox = createCombox(m_cutPanel, itemList);
						m_modeComboBox->Bind(wxEVT_COMBOBOX, &GLGizmoCut3D::OnModeComboBoxChanged, this);
						modeSizer->Add(m_modeComboBox, 1, wxEXPAND | wxALL, 10);
					}

					// Build Volume
					{
						wxBoxSizer* buildVolumeSizer = new wxBoxSizer(wxHORIZONTAL);
						cutPanelSizer->Add(buildVolumeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_buildVolumeText = new wxStaticText(m_cutPanel, wxID_ANY, _L("Build Volume")/*engin_cut_build_volume*/);
						m_buildVolumeText->SetFont(ANKER_FONT_NO_1);
						m_buildVolumeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						buildVolumeSizer->Add(m_buildVolumeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						buildVolumeSizer->Add(new wxStaticText(m_cutPanel, wxID_ANY, " "), 1, wxEXPAND | wxALL, 0);

						m_buildVolumeTextSize = new  wxStaticText(m_cutPanel, wxID_ANY, "");
						buildVolumeSizer->Add(m_buildVolumeTextSize, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						m_buildVolumeTextSize->SetFont(ANKER_FONT_NO_2);
						m_buildVolumeTextSize->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
						m_buildVolumeTextSize->SetMinSize(AnkerSize(260, ctrlHeight - 2));
						m_buildVolumeTextSize->SetMaxSize(AnkerSize(260, ctrlHeight - 2));
					}

					// Cut Position
					{
						cutPanelSizer->AddSpacer(10);
						wxBoxSizer* cutPositionSizer = new wxBoxSizer(wxHORIZONTAL);
						cutPanelSizer->Add(cutPositionSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_cutPositionText = new  wxStaticText(m_cutPanel, wxID_ANY, _L("Cut Position")/*engin_cut_position*/);
						m_cutPositionText->SetFont(ANKER_FONT_NO_1);
						m_cutPositionText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						cutPositionSizer->Add(m_cutPositionText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						cutPositionSizer->Add(new wxStaticText(m_cutPanel, wxID_ANY, "      "), 1, wxEXPAND | wxALL, 0);
						m_cutPositionResetBtn = new ScalableButton(m_cutPanel, wxID_ANY, "reset_btn", "", AnkerSize(20, 20));
						m_cutPositionResetBtn->SetMinSize(AnkerSize(ctrlHeight, ctrlHeight));
						m_cutPositionResetBtn->SetMaxSize(AnkerSize(ctrlHeight, ctrlHeight));
						m_cutPositionResetBtn->SetSize(AnkerSize(ctrlHeight, ctrlHeight));
						m_cutPositionResetBtn->SetWindowStyleFlag(wxBORDER_NONE);
						m_cutPositionResetBtn->SetBackgroundColour(wxColour("#292A2D"));
						m_cutPositionResetBtn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event) {
							Plater::TakeSnapshot snapshot(wxGetApp().plater(), _L("Reset cutting plane"), UndoRedo::SnapshotType::GizmoAction);
							reset_cut_plane();
							});
						m_cutPositionResetBtn->Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent& event) {m_cutPositionResetBtn->SetCursor(wxCursor(wxCURSOR_HAND)); });
						m_cutPositionResetBtn->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent& event) {m_cutPositionResetBtn->SetCursor(wxCursor(wxCURSOR_NONE)); });
						cutPositionSizer->Add(m_cutPositionResetBtn, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						cutPositionSizer->AddSpacer(1);
						m_cutPositionZText = new wxStaticText(m_cutPanel, wxID_ANY, "Z:");
						m_cutPositionZText->SetFont(ANKER_FONT_NO_1);
						m_cutPositionZText->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
						m_cutPositionZText->SetMinSize(AnkerSize(15, ctrlHeight - 4));
						m_cutPositionZText->SetMaxSize(AnkerSize(15, ctrlHeight - 4));
						cutPositionSizer->Add(m_cutPositionZText, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						cutPositionSizer->AddSpacer(3);
						m_cutPositionZLineEdit = new AnkerLineEditUnit(m_cutPanel, "", wxFont(ANKER_FONT_NO_1), wxColour(41, 42, 45), wxColour("#3F4043"), 4);
						m_cutPositionZLineEdit->SetValue(PositionZLineEditInitValue);
						m_cutPositionZLineEdit->SetMinSize(AnkerSize(150, ctrlHeight + 5));
						m_cutPositionZLineEdit->SetMaxSize(AnkerSize(150, ctrlHeight + 5));
						m_cutPositionZLineEdit->SetSize(AnkerSize(150, ctrlHeight + 5));
						m_cutPositionZLineEdit->Bind(wxCUSTOMEVT_EDIT_FOCUS, [this](wxCommandEvent& event) {
							m_cutPositionZEditMode = AnkerLineEditUnitMode_Editing;
							});
						m_cutPositionZLineEdit->Bind(wxCUSTOMEVT_EDIT_FINISHED, [this](wxCommandEvent& event) {
							// Edit finished.
							m_cutPositionZEditMode = AnkerLineEditUnitMode_Edited;
							});
						m_cutPositionZLineEdit->getTextEdit()->Bind(wxEVT_CHAR, [this](wxKeyEvent& event) {
							int keyCode = event.GetKeyCode();
							bool canInput = (keyCode >= '0' && keyCode <= '9') ? true : (keyCode == '.') ? 
								true :(keyCode == '+' || keyCode == '-') ? true :
								(keyCode == WXK_BACK || keyCode == WXK_DELETE || keyCode == WXK_RETURN || keyCode == WXK_TAB) ? 
								true : false;
							if (canInput) {
								event.Skip();
							}
						...
						cutPositionSizer->Add(m_cutPositionZLineEdit, 0, wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, wxALIGN_CENTER_VERTICAL, 0);
					}

					// Add Connectors
					{
						cutPanelSizer->AddSpacer(10);
						m_addConnectorBtnSizer = new wxBoxSizer(wxHORIZONTAL);
						cutPanelSizer->Add(m_addConnectorBtnSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_addConnectorsBtn = new AnkerBtn(m_cutPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
						m_addConnectorsBtn->SetRadius(4);
						m_addConnectorsBtn->SetFont(ANKER_FONT_NO_1);
						m_addConnectorsBtn->SetText(_L("Add Connectors")/*engin_cut_add_connectors*/);
						m_addConnectorsBtn->SetBackgroundColour(wxColour("#3A3B3F"));
						m_addConnectorsBtn->SetTextColor(wxColour("#62D361"));
						m_addConnectorsBtn->SetMinSize(AnkerSize(200, ctrlHeight + 4));
						m_addConnectorsBtn->SetMaxSize(AnkerSize(200, ctrlHeight + 4));
						m_addConnectorsBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
							perform_cut_show(false);
							set_connectors_editing(true);
							});

						m_addConnectorBtnSizer->Add(m_addConnectorsBtn, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						m_addConnectorBtnSizer->AddSpacer(8);
						m_resetCutBtn = new AnkerBtn(m_cutPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
						m_resetCutBtn->SetRadius(4);
						m_resetCutBtn->SetFont(ANKER_FONT_NO_1);
						m_resetCutBtn->SetText(_L("Reset Cut")/*engin_cut_reset_cut*/);
						m_resetCutBtn->SetBackgroundColour(wxColour("#3A3B3F"));
						m_resetCutBtn->SetTextColor(wxColour("#62D361"));
						m_resetCutBtn->SetMinSize(AnkerSize(200, ctrlHeight + 4));
						m_resetCutBtn->SetMaxSize(AnkerSize(200, ctrlHeight + 4));
						m_resetCutBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
							Plater::TakeSnapshot snapshot(wxGetApp().plater(), _L("Reset cut"), UndoRedo::SnapshotType::GizmoAction);
							reset_cut_plane();
							reset_connectors();
							});
						m_resetCutBtn->SetStatus(DisableBtnStatus);
						m_addConnectorBtnSizer->Add(m_resetCutBtn, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}
				}

				cutPanelSizer->AddSpacer(10);
				cutPanelSizer->Add(new AnkerLine2(m_cutPanel, wxID_ANY, wxDefaultPosition, AnkerSize(392, 1), wxColour("#38393C")));

				// Groove
				{
					cutPanelSizer->AddSpacer(10);
					m_croovePanel = new wxPanel(m_cutPanel);
					cutPanelSizer->Add(m_croovePanel, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					wxBoxSizer* croovePanelSizer = new wxBoxSizer(wxVERTICAL);
					m_croovePanel->SetSizer(croovePanelSizer);
					m_crooveText = new wxStaticText(m_croovePanel, wxID_ANY, _L("engin_cut_groove")/*Groove*/);
					m_crooveText->SetFont(ANKER_BOLD_FONT_NO_1);
					m_crooveText->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
					croovePanelSizer->Add(m_crooveText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 10);
					m_croovePanel->Show(false);

					{
						{
							wxBoxSizer* depthSizer = new wxBoxSizer(wxHORIZONTAL);
							croovePanelSizer->Add(depthSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 0);
							m_depthText = new wxStaticText(m_croovePanel, wxID_ANY, _L("engin_cut_depth")/*Depth*/);
							m_depthText->SetFont(ANKER_FONT_NO_1);
							m_depthText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
							depthSizer->Add(m_depthText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);

							std::array<double, 2> floatValueRange{ 2.0f, 39.0 };
							double sizeInit = 6.81;
							m_groove.depth_init = sizeInit;
							auto valueProp = std::make_shared<_Property>(m_croovePanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							m_depthValueSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_groove.depth = value;
								update_plane_model();
								reset_cut_by_contours();
								}, false);
							depthSizer->Add(m_depthValueSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);

							std::array<double, 2> floatToleranceRange{ 0.001f, 0.199f };
							double toleranceInit = 0.1;
							auto toleranceProp = std::make_shared<_Property>(m_croovePanel, "mm", wxString::Format(wxT("%.2f"), toleranceInit), toleranceInit, floatToleranceRange, true);
							m_depthToleranceSlider = std::make_unique<SliderBoxSizerWrapper>(container, toleranceProp, [&](double value) {
								m_groove.depth_tolerance = value;
								update_plane_model();
								reset_cut_by_contours();
								}, false);
							depthSizer->Add(m_depthToleranceSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
						}


						{
							std::array<double, 2> floatValueRange{ 2.001f, 45.41f };
							double sizeInit = 27.24;
							m_groove.width_init = sizeInit;
							auto valueProp = std::make_shared<_Property>(m_croovePanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							wxBoxSizer* widthSizer = new wxBoxSizer(wxHORIZONTAL);
							croovePanelSizer->Add(widthSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 0);
							m_widthText = new wxStaticText(m_croovePanel, wxID_ANY, _L("engin_cut_width")/*Width*/);
							m_widthText->SetFont(ANKER_FONT_NO_1);
							m_widthText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
							widthSizer->Add(m_widthText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
							m_widthValueSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_groove.width = value;
								update_plane_model();
								reset_cut_by_contours();
								}, false);
							widthSizer->Add(m_widthValueSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);

							std::array<double, 2> floatToleranceRange{ 0.001f, 1.999f };
							double toleranceInit = 0.1;
							auto toleranceProp = std::make_shared<_Property>(m_croovePanel, "mm", wxString::Format(wxT("%.2f"), toleranceInit), toleranceInit, floatToleranceRange, true);
							m_widthToleranceSlider = std::make_unique<SliderBoxSizerWrapper>(container, toleranceProp, [&](double value) {
								m_groove.width_tolerance = value;
								update_plane_model();
								reset_cut_by_contours();
								}, false);
							widthSizer->Add(m_widthToleranceSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
						}


						{
							wxBoxSizer* flapAngleSizer = new wxBoxSizer(wxHORIZONTAL);
							croovePanelSizer->Add(flapAngleSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 0);
							m_flapAngleText = new wxStaticText(m_croovePanel, wxID_ANY, _L("Flap Angle")/*engin_cut_flap_angle*/);
							m_flapAngleText->SetFont(ANKER_FONT_NO_1);
							m_flapAngleText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
							flapAngleSizer->Add(m_flapAngleText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
							int angleInit = 60;
							m_groove.angle_init = deg2rad(angleInit);
							std::array<double, 2> intValueRange{ 30.1f, 119.9f };
							auto angleProp = std::make_shared<_Property>(m_croovePanel, wxString::FromUTF8("°"), wxString::Format(wxT("%d"), angleInit), (double)angleInit, intValueRange, true);
							m_flapAngleSlider = std::make_unique <SliderBoxSizerWrapper>(container, angleProp, [&](double value) {
								m_groove.flaps_angle = deg2rad(value);
								update_plane_model();
								reset_cut_by_contours();
								}, true);
							flapAngleSizer->Add(m_flapAngleSlider->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						}

						{
							wxBoxSizer* grooveAngleSizer = new wxBoxSizer(wxHORIZONTAL);
							croovePanelSizer->Add(grooveAngleSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 0);
							m_grooveAngleText = new wxStaticText(m_croovePanel, wxID_ANY, _L("Groove Angle")/*engin_cut_groove_angle*/);
							m_grooveAngleText->SetFont(ANKER_FONT_NO_1);
							m_grooveAngleText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
							grooveAngleSizer->Add(m_grooveAngleText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
							int angleInit = 0;
							m_groove.angle_init = deg2rad(angleInit);
							std::array<double, 2> intValueRange{ 0.f, 14.99f };
							auto angleProp2 = std::make_shared<_Property>(m_croovePanel, wxString::FromUTF8("°"), wxString::Format(wxT("%d"), angleInit), angleInit, intValueRange, true);
							m_grooveAngleSlider = std::make_unique<SliderBoxSizerWrapper>(container, angleProp2, [&](double value) {
								m_groove.angle = deg2rad(value);
								update_plane_model();
								reset_cut_by_contours();
								}, true);
							grooveAngleSizer->Add(m_grooveAngleSlider->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						}

					}
					croovePanelSizer->AddSpacer(10);
					croovePanelSizer->Add(new AnkerLine2(m_croovePanel, wxID_ANY, wxDefaultPosition, AnkerSize(392, 1), wxColour("#38393C")));
				}

				// Cut Result
				{
					m_resultPanel = new wxPanel(m_cutPanel);
					cutPanelSizer->Add(m_resultPanel, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					wxBoxSizer* resultPanelSizer = new wxBoxSizer(wxVERTICAL);
					m_resultPanel->SetSizer(resultPanelSizer);
					resultPanelSizer->AddSpacer(10);
					m_cutResultText = new wxStaticText(m_resultPanel, wxID_ANY, _L("Cut Result")/*engin_cut_result*/);
					m_cutResultText->SetFont(ANKER_BOLD_FONT_NO_1);
					m_cutResultText->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
					resultPanelSizer->Add(m_cutResultText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 10);

					{
						wxImage uncheckImage = wxImage(wxString::FromUTF8(Slic3r::var("checkbox_uncheck.png")), wxBITMAP_TYPE_PNG);
						uncheckImage.Rescale(ctrlHeight, ctrlHeight, wxIMAGE_QUALITY_HIGH);
						...
						checkImage.Rescale(ctrlHeight, ctrlHeight, wxIMAGE_QUALITY_HIGH);
						...
						disuncheckImage.Rescale(ctrlHeight, ctrlHeight, wxIMAGE_QUALITY_HIGH);
						...
						discheckImage.Rescale(ctrlHeight, ctrlHeight, wxIMAGE_QUALITY_HIGH);
						wxBitmap disCheckScaledBitmap(discheckImage);

						wxCursor handCursor(wxCURSOR_HAND);

						resultPanelSizer->AddSpacer(10);
						wxBoxSizer* checkBoxBigSizer = new wxBoxSizer(wxHORIZONTAL);
						resultPanelSizer->Add(checkBoxBigSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 0);

						wxBoxSizer* resultASizer = new  wxBoxSizer(wxVERTICAL);
						checkBoxBigSizer->Add(resultASizer, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);

						wxBoxSizer* objectChooseBtnASizer = new  wxBoxSizer(wxHORIZONTAL);
						resultASizer->Add(objectChooseBtnASizer, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						resultASizer->AddSpacer(10);
						m_objectACheckBox = new AnkerCheckBox(m_resultPanel,
							...
							_L("engin_cut_object")/*Object*/ + wxString(" A"),
							ANKER_FONT_NO_1,
							wxColour("#FFFFFF"),
							wxID_ANY);
						m_objectACheckBox->SetCursor(handCursor);
						m_objectACheckBox->SetWindowStyleFlag(wxBORDER_NONE);
						m_objectACheckBox->SetBackgroundColour(wxColour("#292A2D"));
						m_objectACheckBox->setBgColor(wxColour("#292A2D"));
						m_objectACheckBox->SetMinSize(AnkerSize(90, ctrlHeight));
						m_objectACheckBox->SetMaxSize(AnkerSize(90, ctrlHeight));
						m_objectACheckBox->SetSize(AnkerSize(90, ctrlHeight));
						m_objectACheckBox->setCheckStatus(true);
						objectChooseBtnASizer->Add(m_objectACheckBox, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						m_objectACheckBox->Bind(wxCUSTOMEVT_ANKER_CHECKBOX_CLICKED, [this](wxCommandEvent& event)
							{
								if (m_objectARadioBoxSizer && m_addConnectorsBtn) {
									bool enable = m_objectACheckBox->getCheckStatus();
									m_keep_upper = enable;
									m_objectARadioBoxSizer->set_enable(enable);
									m_addConnectorsBtn->Enable(enable);
									container->Layout();
								}

							});
						objectChooseBtnASizer->AddSpacer(10);
						//m_objectChooseBtnA = new AnkerSwitchButton(m_resultPanel, wxID_ANY, _L("Object A")/*engin_cut_object_a*/, "#A9AAAB", ANKER_FONT_NO_1, "#292A2D");

						wxPanel* objectChooseBtnAPanel = new wxPanel(m_resultPanel);
						objectChooseBtnAPanel->SetBackgroundColour("#00FFFF");
						objectChooseBtnAPanel->SetMinSize(AnkerSize(ctrlHeight, ctrlHeight));
						objectChooseBtnAPanel->SetMaxSize(AnkerSize(ctrlHeight, ctrlHeight));
						objectChooseBtnASizer->Add(objectChooseBtnAPanel, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);

						std::vector<wxString> objectARadioLabels;
						objectARadioLabels.push_back(_L("Keep Orientation")/*engin_cut_keep_orientation*/);
						objectARadioLabels.push_back(_L("Place On Cut")/*engin_cut_place_on_cut*/);
						objectARadioLabels.push_back(_L("Flip Upside Down")/*engin_cut_flip_upside_down*/);
						m_objectARadioBoxSizer = std::make_unique<RadioBoxSizerWrapper>(m_resultPanel, objectARadioLabels, [&](size_t value) {
							switch (value) {
							case 0:
								m_keep_upper = true;
						...
						m_objectARadioBoxSizer->select_item();
						resultASizer->Add(m_objectARadioBoxSizer->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);

						wxBoxSizer* resultBSizer = new  wxBoxSizer(wxVERTICAL);
						wxBoxSizer* objectChooseBtnBSizer = new  wxBoxSizer(wxHORIZONTAL);
						resultBSizer->Add(objectChooseBtnBSizer, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						resultBSizer->AddSpacer(10);
						m_objectBCheckBox = new AnkerCheckBox(m_resultPanel,
							uncheckScaledBitmap.ConvertToImage(),
							checkScaledBitmap.ConvertToImage(),
							disUncheckScaledBitmap.ConvertToImage(),
							disCheckScaledBitmap.ConvertToImage(),
							_L("engin_cut_object")/*Object*/ + wxString(" B"),
							ANKER_FONT_NO_1,
							wxColour("#FFFFFF"),
							wxID_ANY);
						m_objectBCheckBox->SetCursor(handCursor);
						m_objectBCheckBox->SetWindowStyleFlag(wxBORDER_NONE);
						m_objectBCheckBox->SetBackgroundColour(wxColour("#292A2D"));
						m_objectBCheckBox->setBgColor(wxColour("#292A2D"));
						m_objectBCheckBox->SetMinSize(AnkerSize(90, ctrlHeight));
						m_objectBCheckBox->SetMaxSize(AnkerSize(90, ctrlHeight));
						m_objectBCheckBox->SetSize(AnkerSize(90, ctrlHeight));
						m_objectBCheckBox->setCheckStatus(true);
						objectChooseBtnBSizer->Add(m_objectBCheckBox, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						m_objectBCheckBox->Bind(wxCUSTOMEVT_ANKER_CHECKBOX_CLICKED, [this](wxCommandEvent& event)
							{
								if (m_objectBRadioBoxSizer && m_addConnectorsBtn) {
									bool enable = m_objectBCheckBox->getCheckStatus();
									m_keep_lower = enable;
									m_objectBRadioBoxSizer->set_enable(enable);
									m_addConnectorsBtn->Enable(enable);
									container->Layout();
								}
							});
						objectChooseBtnBSizer->AddSpacer(10);
						wxPanel* objectChooseBtnBPanel = new wxPanel(m_resultPanel);
						objectChooseBtnBPanel->SetBackgroundColour("#FF00FF");
						objectChooseBtnBPanel->SetMinSize(AnkerSize(ctrlHeight, ctrlHeight));
						objectChooseBtnBPanel->SetMaxSize(AnkerSize(ctrlHeight, ctrlHeight));
						objectChooseBtnBSizer->Add(objectChooseBtnBPanel, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						m_objectBRadioBoxSizer = std::make_unique<RadioBoxSizerWrapper>(m_resultPanel, objectARadioLabels, [&](size_t value) {
							switch (value) {
							case 0:
						...
						m_objectBRadioBoxSizer->select_item(1);
						resultBSizer->Add(m_objectBRadioBoxSizer->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						checkBoxBigSizer->AddSpacer(20);
						checkBoxBigSizer->Add(resultBSizer, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}
				...
				// Cut into
				{
					m_intoPanel = new wxPanel(m_cutPanel);
					cutPanelSizer->Add(m_intoPanel, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					wxBoxSizer* intoPanelSizer = new wxBoxSizer(wxVERTICAL);
					m_intoPanel->SetSizer(intoPanelSizer);

					m_cutIntoText = new wxStaticText(m_intoPanel, wxID_ANY, _L("Cut Into")/*engin_cut_into*/);
					m_cutIntoText->SetFont(ANKER_BOLD_FONT_NO_1);
					m_cutIntoText->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
					intoPanelSizer->Add(m_cutIntoText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 10);
					intoPanelSizer->AddSpacer(5);
					std::vector<wxString> cutIntoLabels;
					cutIntoLabels.push_back(_L("engin_cut_objects")/*Objects*/);
					cutIntoLabels.push_back(_L("engin_cut_parts")/*Parts*/);
					m_cutIntoRadioBoxSizer = std::make_unique<RadioBoxSizerWrapper>(m_intoPanel, cutIntoLabels, [&](size_t value) {
						if (value == 0) {
							m_keep_as_parts = false;
						...
						bool has_connectors = !m_c->selection_info()->model_object()->cut_connectors.empty();
						if (m_objectACheckBox && m_objectBCheckBox) {
							if (m_keep_as_parts) {
								m_objectACheckBox->setText(_L("engin_cut_part")/*Part*/ + wxString(" A"));
								m_objectBCheckBox->setText(_L("engin_cut_part") + wxString(" B"));
							}
							else {
								m_objectACheckBox->setText(_L("engin_cut_object") + wxString(" A"));
								m_objectBCheckBox->setText(_L("engin_cut_object")+ wxString(" B"));
							}
							if (has_connectors) {
								m_objectACheckBox->Enable(true);
								m_objectBCheckBox->Enable(true);
								m_objectARadioBoxSizer->set_enable(true);
								m_objectBRadioBoxSizer->set_enable(true);
							}
							else {
								m_objectACheckBox->Enable(!m_keep_as_parts && m_keep_upper);
								m_objectBCheckBox->Enable(!m_keep_as_parts && m_keep_lower);
								m_objectARadioBoxSizer->set_enable(!m_keep_as_parts && m_keep_upper);
								m_objectBRadioBoxSizer->set_enable(!m_keep_as_parts && m_keep_lower);
							}
					...
					m_cutIntoRadioBoxSizer->select_item();
					intoPanelSizer->Add(m_cutIntoRadioBoxSizer->get_boxSizer(), 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
				}
				...
				{
					cutPanelSizer->AddSpacer(10);
					m_performCutBtn = new AnkerBtn(m_cutPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
					cutPanelSizer->Add(m_performCutBtn, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					m_performCutBtn->SetRadius(4);
					m_performCutBtn->SetFont(ANKER_FONT_NO_1);
					m_performCutBtn->SetText(_L("Perform Cut")/*engin_cut_perform_cut*/);
					m_performCutBtn->SetBackgroundColour(wxColour("#3A3B3F"));
					m_performCutBtn->SetTextColor(wxColour("#62D361"));
					m_performCutBtn->SetMinSize(AnkerSize(392, ctrlHeight + 4));
					m_performCutBtn->SetMaxSize(AnkerSize(392, ctrlHeight + 4));
					m_performCutBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
						perform_cut(m_parent.get_selection());
						});
				}
				...
				{
					cutPanelSizer->AddSpacer(10);
					wxBoxSizer *warningInvalidCutInfoSizer = new wxBoxSizer(wxHORIZONTAL);
					cutPanelSizer->Add(warningInvalidCutInfoSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					m_warningInvalidCutInfoImage = new ImagePanel(m_cutPanel, wxString::FromUTF8(Slic3r::var("hms_notify_lv2.png")), wxBITMAP_TYPE_PNG);
					m_warningInvalidCutInfoImage->SetMinSize(AnkerSize(ctrlHeight - 2, ctrlHeight - 2));
					m_warningInvalidCutInfoImage->SetMaxSize(AnkerSize(ctrlHeight - 2, ctrlHeight - 2));
					warningInvalidCutInfoSizer->Add(m_warningInvalidCutInfoImage, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					warningInvalidCutInfoSizer->AddSpacer(10);
					int width = 365;
					m_invalidCutInfoText = new wxStaticText(m_cutPanel, wxID_ANY, "");
					m_invalidCutInfoText->SetFont(ANKER_BOLD_FONT_NO_1);
					m_invalidCutInfoText->Wrap(AnkerSize(width, ctrlHeight).GetWidth());
					m_invalidCutInfoText->SetForegroundColour(wxColour("#FF6F00"));
					m_invalidCutInfoText->SetMinSize(AnkerSize(width, ctrlHeight));
					m_invalidCutInfoText->SetMaxSize(AnkerSize(width, ctrlHeight));
					warningInvalidCutInfoSizer->Add(m_invalidCutInfoText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					m_warningInvalidCutInfoImage->visible = false;
				}


				// Connectors
				{
					m_connectorsPanel = new wxPanel(container);
					wxBoxSizer* connectorsSizer = new wxBoxSizer(wxVERTICAL);
					m_connectorsPanel->SetSizer(connectorsSizer);
					connectorsSizer->AddSpacer(10);
					m_connectorsPanel->Show(false);
					wxBoxSizer* connectorsFlipSizer = new wxBoxSizer(wxHORIZONTAL);
					connectorsSizer->Add(connectorsFlipSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
					m_connectorsText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("Connectors")/*engin_cut_connectors*/);
					m_connectorsText->SetFont(ANKER_BOLD_FONT_NO_1);
					m_connectorsText->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
					m_connectorsText->SetMinSize(AnkerSize(85, ctrlHeight));
					m_connectorsText->SetMaxSize(AnkerSize(85, ctrlHeight));
					connectorsFlipSizer->Add(m_connectorsText, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					connectorsFlipSizer->AddSpacer(5);
					m_connectorsResetBtn = new ScalableButton(m_connectorsPanel, wxID_ANY, "reset_btn", "", AnkerSize(ctrlHeight, ctrlHeight));
					m_connectorsResetBtn->SetMinSize(AnkerSize(ctrlHeight, ctrlHeight));
					m_connectorsResetBtn->SetMaxSize(AnkerSize(ctrlHeight, ctrlHeight));
					m_connectorsResetBtn->SetSize(AnkerSize(ctrlHeight, ctrlHeight));
					m_connectorsResetBtn->SetWindowStyleFlag(wxBORDER_NONE);
					m_connectorsResetBtn->SetBackgroundColour(wxColour("#292A2D"));
					m_connectorsResetBtn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event) {
						Plater::TakeSnapshot snapshot(wxGetApp().plater(), _L("Remove connectors"), UndoRedo::SnapshotType::GizmoAction);
						reset_connectors();
						});
					m_connectorsResetBtn->Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent& event) {m_connectorsResetBtn->SetCursor(wxCursor(wxCURSOR_HAND)); });
					m_connectorsResetBtn->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent& event) {m_connectorsResetBtn->SetCursor(wxCursor(wxCURSOR_NONE)); });
					connectorsFlipSizer->Add(m_connectorsResetBtn, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					connectorsFlipSizer->AddSpacer(50);

					m_flipCutPlaneBtn = new AnkerBtn(m_connectorsPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
					connectorsFlipSizer->Add(m_flipCutPlaneBtn, 0, wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, 10);
					m_flipCutPlaneBtn->SetRadius(4);
					m_flipCutPlaneBtn->SetFont(ANKER_FONT_NO_1);
					m_flipCutPlaneBtn->SetText(_L("Flip Cut Plane")/*engin_cut_flip_cut_plane*/);
					m_flipCutPlaneBtn->SetBackgroundColour(wxColour("#3A3B3F"));
					m_flipCutPlaneBtn->SetTextColor(wxColour("#62D361"));
					m_flipCutPlaneBtn->SetMinSize(AnkerSize(192, ctrlHeight + 4));
					m_flipCutPlaneBtn->SetMaxSize(AnkerSize(192, ctrlHeight + 4));
					m_flipCutPlaneBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
						flip_cut_plane();
						});
					m_flipCutPlaneBtn->Enable(false);

					connectorsSizer->AddSpacer(10);
					connectorsSizer->Add(new AnkerLine2(m_connectorsPanel, wxID_ANY, wxDefaultPosition, AnkerSize(392, 1), wxColour("#38393C")));
					...
					// Type
					{
						...
						connectorsSizer->Add(typeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_typeText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_type")/*Type*/);
						m_typeText->SetFont(ANKER_FONT_NO_1);
						m_typeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						m_typeText->SetMinSize(AnkerSize(100, ctrlHeight));
						m_typeText->SetMaxSize(AnkerSize(100, ctrlHeight));
						typeSizer->Add(m_typeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						std::vector<wxString> typeLabels;
						typeLabels.push_back(_L("engin_cut_plug")/*Plug*/);
						typeLabels.push_back(_L("engin_cut_dowel")/*Dowel*/);
						typeLabels.push_back(_L("engin_cut_snap")/*Snap*/);
						m_connectorsTypeRadioBoxSizer = std::make_unique<RadioBoxSizerWrapper>(m_connectorsPanel, typeLabels, [&](size_t value) {
							m_connector_type = CutConnectorType(value);
							CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
							apply_selected_connectors([this, &connectors](size_t idx) { connectors[idx].attribs.type = CutConnectorType(m_connector_type); });
							if (m_connector_type == CutConnectorType::Dowel) {
								m_connector_style = int(CutConnectorStyle::Prism);
								if (m_styleComboBox) m_styleComboBox->Select(m_connector_style);
							}
							if (m_connector_type == CutConnectorType::Snap) {
								m_connector_shape_id = int(CutConnectorShape::Circle);
								if (m_shapeComboBox) m_shapeComboBox->Select(m_connector_shape_id);
							}


							switch (m_connector_type) {
							case CutConnectorType::Plug:
								if (m_styleComboBox) m_styleComboBox->Enable(true);
								if (m_shapeComboBox) m_shapeComboBox->Enable(true);
								if (m_connectorsBulgeSlider) m_connectorsBulgeSlider->set_enable(false);
								if (m_connectorsSpaceSlider) m_connectorsSpaceSlider->set_enable(false);
								break;
							case CutConnectorType::Dowel:
								if (m_styleComboBox) m_styleComboBox->Enable(false);
								if (m_shapeComboBox) m_shapeComboBox->Enable(true);
								if (m_connectorsBulgeSlider) m_connectorsBulgeSlider->set_enable(false);
								if (m_connectorsSpaceSlider) m_connectorsSpaceSlider->set_enable(false);
								break;
							case CutConnectorType::Snap:
								if (m_styleComboBox) m_styleComboBox->Enable(false);
								if (m_shapeComboBox) m_shapeComboBox->Enable(false);
								if (m_connectorsBulgeSlider) m_connectorsBulgeSlider->set_enable(true);
								if (m_connectorsSpaceSlider) m_connectorsSpaceSlider->set_enable(true);
								break;
							default:break;
							}
							container->Refresh();
							}, RadioBoxSizerType_Horizontal, true);
						m_connectorsTypeRadioBoxSizer->select_item();
						typeSizer->Add(m_connectorsTypeRadioBoxSizer->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
					}
					...
					// Style
					{
						...
						connectorsSizer->Add(styleSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_styleText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_style")/*Style*/);
						m_styleText->SetFont(ANKER_FONT_NO_1);
						m_styleText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						styleSizer->Add(m_styleText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						styleSizer->Add(new wxStaticText(m_connectorsPanel, wxID_ANY, "                  "), 1, wxEXPAND | wxALL, 0);

						wxArrayString itemList;
						itemList.Add(_L("Prism")/*engin_cut_prism*/);
						itemList.Add(_L("Frustum")/*engin_cut_frustum*/);
						m_styleComboBox = createCombox(m_connectorsPanel, itemList);
						m_styleComboBox->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& event) {
							wxOwnerDrawnComboBox* comboBox = wxDynamicCast(event.GetEventObject(), wxOwnerDrawnComboBox);
							if (comboBox) {
								m_connector_style = comboBox->GetSelection();
								CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
								apply_selected_connectors([this, &connectors](size_t idx) { connectors[idx].attribs.style = CutConnectorStyle(m_connector_style); });
							}
							});
						styleSizer->Add(m_styleComboBox, 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
					...
					// Shape
					{
						...
						connectorsSizer->Add(shapeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_shapeText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_shape")/*Shape*/);
						m_shapeText->SetFont(ANKER_FONT_NO_1);
						m_shapeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						shapeSizer->Add(m_shapeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						shapeSizer->Add(new wxStaticText(m_connectorsPanel, wxID_ANY, "                  "), 1, wxEXPAND | wxALL, 0);

						wxArrayString itemList;
						itemList.Add(_L("engin_cut_triangle")/*Triangle*/);
						itemList.Add(_L("engin_cut_square")/*Square*/);
						itemList.Add(_L("engin_cut_hexagon")/*Hexagon*/);
						itemList.Add(_L("engin_cut_circle")/*Circle*/);
						m_shapeComboBox = createCombox(m_connectorsPanel, itemList);
						m_shapeComboBox->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& event) {
							wxOwnerDrawnComboBox* comboBox = wxDynamicCast(event.GetEventObject(), wxOwnerDrawnComboBox);
							if (comboBox) {
								m_connector_shape_id = comboBox->GetSelection();
								CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
								apply_selected_connectors([this, &connectors](size_t idx) { connectors[idx].attribs.shape = CutConnectorShape(m_connector_shape_id); });
							}
							});
						m_shapeComboBox->SetSelection(3);
						shapeSizer->Add(m_shapeComboBox, 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
					}
					...
					{
						wxBoxSizer* depthSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(depthSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsDepthText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_depth")/*Depth*/);
						m_connectorsDepthText->SetFont(ANKER_FONT_NO_1);
						m_connectorsDepthText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						depthSizer->Add(m_connectorsDepthText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						{
							std::array<double, 2> floatValueRange{ 1.001f, 7.829f };
							double sizeInit = 3.00;
							auto valueProp = std::make_shared<_Property>(m_connectorsPanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							m_connectorsDepthValueSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_connector_depth_ratio = value;
								CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
								apply_selected_connectors([this, &connectors](size_t idx) {
									if (m_connector_depth_ratio > 0)
										connectors[idx].height = m_connector_depth_ratio;
							...
							depthSizer->Add(m_connectorsDepthValueSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);

						}

						{
							std::array<double, 2> floatValueRange{ 0.001f, 3.919f };
							double sizeInit = 0.1;
							auto valueProp = std::make_shared<_Property>(m_connectorsPanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							m_connectorsDepthToleranceSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_connector_depth_ratio_tolerance = value;
							...
							depthSizer->Add(m_connectorsDepthToleranceSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
					...
					// Size
					{
						...
						connectorsSizer->Add(sizeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsSizeText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_size")/*Size*/);
						m_connectorsSizeText->SetFont(ANKER_FONT_NO_1);
						m_connectorsSizeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						sizeSizer->Add(m_connectorsSizeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						{
							std::array<double, 2> floatValueRange{ 1.001f, 7.829f };
							double sizeInit = 2.50;
							auto valueProp = std::make_shared<_Property>(m_connectorsPanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							m_connectorsSizeValueSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_connector_size = value;
								CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
								apply_selected_connectors([this, &connectors](size_t idx) {
									if (m_connector_size > 0)
										connectors[idx].radius = 0.5f * m_connector_size;
							...
							sizeSizer->Add(m_connectorsSizeValueSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);

						}

						{
							std::array<double, 2> floatValueRange{ 0.001f, 3.920f };
							double sizeInit = 0.001f;
							auto valueProp = std::make_shared<_Property>(m_connectorsPanel, "mm", wxString::Format(wxT("%.2f"), sizeInit), sizeInit, floatValueRange, true);
							m_connectorsSizeToleranceSlider = std::make_unique<SliderBoxSizerWrapper>(container, valueProp, [&](double value) {
								m_connector_size_tolerance = value;
								CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
								apply_selected_connectors([this, &connectors](size_t idx) {
									if (m_connector_size_tolerance >= 0)
										connectors[idx].radius_tolerance = 0.5f * m_connector_size_tolerance;
							...
							sizeSizer->Add(m_connectorsSizeToleranceSlider->get_boxSizer(), 0, wxEXPAND | wxALIGN_CENTER | wxLEFT | wxRIGHT, 0);
					...
					// Rotation
					{
						wxBoxSizer* rotationAngleSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(rotationAngleSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsRotationText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("Rotation")/*engin_cut_rotation*/);
						m_connectorsRotationText->SetFont(ANKER_FONT_NO_1);
						m_connectorsRotationText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						rotationAngleSizer->Add(m_connectorsRotationText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						int angleInit = 1;
						std::array<double, 2> intValueRange{ 0.1f, 179.9f };
						auto angleProp = std::make_shared<_Property>(m_connectorsPanel, wxString::FromUTF8("°"), wxString::Format(wxT("%d"), angleInit), angleInit, intValueRange, true);
						m_connectorsRotationValueSlider = std::make_unique<SliderBoxSizerWrapper>(container, angleProp, [&](double value) {
							m_connector_angle = deg2rad(value);
							CutConnectors& connectors = m_c->selection_info()->model_object()->cut_connectors;
							apply_selected_connectors([this, &connectors](size_t idx) {
								connectors[idx].z_angle = m_connector_angle;
								});
							}, true);
						rotationAngleSizer->Add(m_connectorsRotationValueSlider->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}

					// Bulge
					{
						wxBoxSizer* connectorsBulgeSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(connectorsBulgeSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsBulgeText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("Bulge")/*engin_cut_bulge*/);
						m_connectorsBulgeText->SetFont(ANKER_FONT_NO_1);
						m_connectorsBulgeText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						connectorsBulgeSizer->Add(m_connectorsBulgeText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						int angleInit = 15;
						std::array<double, 2> intValueRange{ 5.001f, 29.999f };
						auto angleProp = std::make_shared<_Property>(m_connectorsPanel, wxString::FromUTF8("%"), wxString::Format(wxT("%d"), angleInit), angleInit, intValueRange, true);
						m_connectorsBulgeSlider = std::make_unique<SliderBoxSizerWrapper>(container, angleProp, [&](double value) {
							m_snap_bulge_proportion = value / 100.0;
							}, true);
						m_connectorsBulgeSlider->set_enable(false);
						connectorsBulgeSizer->Add(m_connectorsBulgeSlider->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}

					// Space
					{
						wxBoxSizer* connectorsSpaceSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(connectorsSpaceSizer, 0.001, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsSpaceText = new wxStaticText(m_connectorsPanel, wxID_ANY, _L("engin_cut_space")/*Space*/);
						m_connectorsSpaceText->SetFont(ANKER_FONT_NO_1);
						m_connectorsSpaceText->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
						connectorsSpaceSizer->Add(m_connectorsSpaceText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						int angleInit = 30;
						std::array<double, 2> intValueRange{ 10.001f, 49.999f };
						auto angleProp = std::make_shared<_Property>(m_connectorsPanel, wxString::FromUTF8("%"), wxString::Format(wxT("%d"), angleInit), angleInit, intValueRange, true);
						m_connectorsSpaceSlider = std::make_unique<SliderBoxSizerWrapper>(container, angleProp, [&](double value) {
							m_snap_space_proportion = value / 100.0;
							}, true);
						m_connectorsSpaceSlider->set_enable(false);
						connectorsSpaceSizer->Add(m_connectorsSpaceSlider->get_boxSizer(), 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}
					...
					// Cancel & Confirm Connectors
					{
						wxBoxSizer* cancelBtnSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(cancelBtnSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_connectorsCancelBtn = new AnkerBtn(m_connectorsPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
						m_connectorsCancelBtn->SetRadius(4);
						m_connectorsCancelBtn->SetFont(ANKER_FONT_NO_1);
						m_connectorsCancelBtn->SetText(_L("Cancel")/*engin_cut_cancel*/);
						m_connectorsCancelBtn->SetBackgroundColour(wxColour("#3A3B3F"));
						m_connectorsCancelBtn->SetTextColor(wxColour("#62D361"));
						m_connectorsCancelBtn->SetMinSize(AnkerSize(190, ctrlHeight + 4));
						m_connectorsCancelBtn->SetMaxSize(AnkerSize(190, ctrlHeight + 4));
						m_connectorsCancelBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
							perform_cut_show(true);
							reset_connectors();
							set_connectors_editing(false);
							container->Refresh();
							});

						cancelBtnSizer->Add(m_connectorsCancelBtn, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						cancelBtnSizer->AddSpacer(5);
						m_connectorsConfirmConnectorsBtn = new AnkerBtn(m_connectorsPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
						m_connectorsConfirmConnectorsBtn->SetRadius(4);
						m_connectorsConfirmConnectorsBtn->SetFont(ANKER_FONT_NO_1);
						m_connectorsConfirmConnectorsBtn->SetText(_L("Confirm Connectors")/*engin_cut_confirm_connectors*/);
						m_connectorsConfirmConnectorsBtn->SetBackgroundColour(wxColour("#3A3B3F"));
						m_connectorsConfirmConnectorsBtn->SetTextColor(wxColour("#62D361"));
						m_connectorsConfirmConnectorsBtn->SetMinSize(AnkerSize(190, ctrlHeight + 4));
						m_connectorsConfirmConnectorsBtn->SetMaxSize(AnkerSize(190, ctrlHeight + 4));
						m_connectorsConfirmConnectorsBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
							perform_cut_show(true);
							unselect_all_connectors();
							set_connectors_editing(false);
							container->Refresh();
							});
						cancelBtnSizer->Add(m_connectorsConfirmConnectorsBtn, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}

					// Invalid connectors detected:-1 connector is out of object.
					{
						connectorsSizer->AddSpacer(10);
						wxBoxSizer *warningInvalidConnectorsSizer = new wxBoxSizer(wxHORIZONTAL);
						connectorsSizer->Add(warningInvalidConnectorsSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
						m_warningInvalidConnectorsImage = new ImagePanel(m_connectorsPanel, wxString::FromUTF8(Slic3r::var("hms_notify_lv2.png")), wxBITMAP_TYPE_PNG);
						m_warningInvalidConnectorsImage->SetMinSize(AnkerSize(ctrlHeight - 2, ctrlHeight - 2));
						m_warningInvalidConnectorsImage->SetMaxSize(AnkerSize(ctrlHeight - 2, ctrlHeight - 2));
						m_warningInvalidConnectorsImage->visible = false;
						warningInvalidConnectorsSizer->Add(m_warningInvalidConnectorsImage, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
						warningInvalidConnectorsSizer->AddSpacer(10);
						m_invalidConnectorsInfoText = new wxStaticText(m_connectorsPanel, wxID_ANY, "");
						m_invalidConnectorsInfoText->SetFont(ANKER_BOLD_FONT_NO_2);
						int width = 365;
						m_invalidConnectorsInfoText->Wrap(AnkerSize(width, ctrlHeight).GetWidth());
						m_invalidConnectorsInfoText->SetForegroundColour(wxColour("#FF6F00"));
						m_invalidConnectorsInfoText->SetMinSize(AnkerSize(width, ctrlHeight * 2 ));
						m_invalidConnectorsInfoText->SetMaxSize(AnkerSize(width, ctrlHeight * 2));
						warningInvalidConnectorsSizer->Add(m_invalidConnectorsInfoText, 1, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
					}
				}
				...
				wxGetApp().plater()->sidebarnew().Bind(wxCUSTOMEVT_MAIN_SIZER_CHANGED, [this](wxCommandEvent& event) {
					event.Skip();
		...
		void GLGizmoCut3D::OnModeComboBoxChanged(wxCommandEvent& event)
		{
			if (m_croovePanel) {
				wxOwnerDrawnComboBox* comboBox = wxDynamicCast(event.GetEventObject(), wxOwnerDrawnComboBox);
				if (comboBox) {
					...
					Plater::TakeSnapshot snapshot(wxGetApp().plater(), _L("Change cut mode"), UndoRedo::SnapshotType::GizmoAction);
					switch_to_mode(size_t(index));
					check_and_update_connectors_state();
					if (index == 0) {
						m_croovePanel->Show(false);
						m_addConnectorBtnSizer->ShowItems(true);
						m_cutIntoRadioBoxSizer->set_enable(true);
					}
					else {
						m_croovePanel->Show(true);
						m_addConnectorBtnSizer->ShowItems(false);
						m_cutIntoRadioBoxSizer->set_enable(false);
					}
					m_intoPanel->Show();
					container->Layout();
				}
		...
		void GLGizmoCut3D::perform_cut_show(bool show)
		{
			if (container && m_cutPanel && m_connectorsPanel) {
				m_connectorsPanel->Show(!show);
				m_cutPanel->Show(show);
				show ? container->setContentPanel(m_cutPanel) : container->setContentPanel(m_connectorsPanel);
				container->Layout();
			}
		}

		AnkerSimpleCombox* GLGizmoCut3D::createCombox(wxPanel* panel, const wxArrayString& itemList)
		{
			...
			AnkerSimpleCombox* comboBox = new AnkerSimpleCombox();
			comboBox->Create(panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, itemList, wxNO_BORDER | wxCB_READONLY);
			comboBox->SetButtonBitmaps(dropBtnBmpNormal, true, dropBtnBmpPressed, dropBtnBmpHover);
			comboBox->SetBackgroundColour(wxColour("#292A2D"));
			comboBox->setColor(wxColour("#434447"), wxColour("#3A3B3F"));
			comboBox->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
			comboBox->SetFont(ANKER_FONT_NO_1);
			comboBox->SetMinSize(wxSize(290, ANKER_COMBOBOX_HEIGHT));
			comboBox->SetSize(wxSize(290, ANKER_COMBOBOX_HEIGHT));
			comboBox->SetSelection(0);
			return comboBox;
		}

		ImageButtonWarpper::ImageButtonWarpper(AnkerTitledPanel* container, wxPanel* panel, const wxString& name,
			const wxString& normalImage, const wxString& pressImage, std::function<void()> action)
		{
			if (!container || !panel || !action) {
				return;
			}

			m_cb = action;
			m_container = container;
			wxImage normal_Image = wxImage(normalImage, wxBITMAP_TYPE_PNG);
			wxImage pressed_Image = wxImage(pressImage, wxBITMAP_TYPE_PNG);
			normal_Image.Rescale(46, 46);
			pressed_Image.Rescale(46, 46);
			m_btn = new wxButton(panel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxNO_BORDER);
			name == BTN_PRESSED ? m_btn->SetBitmap(pressed_Image) : m_btn->SetBitmap(normal_Image);
			m_btn->SetBitmapHover(pressed_Image);
			m_btn->SetMinSize(normal_Image.GetSize());
			m_btn->SetMaxSize(normal_Image.GetSize());
			m_btn->SetSize(normal_Image.GetSize());
			m_btn->SetName(name);
			m_btn->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
			m_pair = std::make_pair(normal_Image, pressed_Image);
			m_btn->Bind(wxEVT_BUTTON, [&](wxCommandEvent& event) {
				if (m_btn->GetName() == BTN_PRESSED) return;
				if (m_cb) { m_cb(); }
				if (m_container) { m_container->Layout(); m_container->Refresh(); }
				});
		}

		ImageButtonWarpper::ImageButtonWarpper(AnkerTitledPanel* container, wxPanel* panel, const wxString& name, std::function<void()> action, bool show)
		{
			if (!container || !panel || !action) {
				return;
			}
			...
			m_cb = action;
			m_container = container;
			m_apply_btn = new AnkerBtn(panel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
			m_apply_btn->SetText(name);
			m_apply_btn->SetMinSize(AnkerSize(170, 32));
			m_apply_btn->SetBackgroundColour(wxColor(97, 98, 101));
			m_apply_btn->SetForegroundColour(wxColour("#FFFFFF"));
			m_apply_btn->SetRadius(3);
			m_apply_btn->SetTextColor(wxColor("#62D361"));
			m_apply_btn->SetFont(ANKER_BOLD_FONT_SIZE(10));
			m_apply_btn->Show(show);
			m_apply_btn->Bind(wxEVT_BUTTON, [&](wxCommandEvent& event) {
				if (m_cb) { m_cb(); }
				if (m_container) { m_container->Layout(); m_container->Refresh(); }
				});
		}

		void ImageButtonWarpper::select()
		{
			if (!m_btn) return;

			if (m_btn->GetName() == BTN_PRESSED) return;
			m_btn->SetBitmap(m_pair.second);
			m_btn->SetBitmapHover(m_pair.second);
			m_btn->SetName(BTN_PRESSED);
		}

		void ImageButtonWarpper::unselect()
		{
			if (!m_btn) {
				return;
			}

			m_btn->SetBitmap(m_pair.first);
			m_btn->SetBitmapHover(m_pair.second);
			m_btn->SetName(BTN_NORMAL);
		}

		SliderBoxSizerWrapper::SliderBoxSizerWrapper(AnkerTitledPanel* container, const std::shared_ptr<_Property>& property, std::function<void(double)> action, bool bAngle)
		{
			if (!property || !container) {
				return;
			}

			if (!property->_panel || property->_range.size() < 2 || !action) {
				return;
			}
			...
			// slider
			m_slider = create_slider(property->_panel, property->_value, property->_range, property->_step);
			m_boxSizer->Add(m_slider, 1, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxALIGN_LEFT | wxTOP, 3);
			m_boxSizer->AddSpacer(2);

			// bursh edit
			m_lineEdit = create_lineEditUnit(property->_panel, property->_unit, property->_textValue, property->_range);
			m_boxSizer->Add(m_lineEdit, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxTOP, 3);
			m_boxSizer->Show(property->_bShow);

			m_slider->Bind(wxANKEREVT_SLIDER_VALUE_CHANGED, [&](wxCommandEvent& event) {
				if (m_isEditing) return;
				m_isEditing = true;
				double currentValue = m_slider->getCurrentValue();
				if (m_cb) {
					m_cb(currentValue);
				}
				char text[10];
				memset(text, 0, 10);
				m_bAngle == true ? sprintf(text, "%d", (int)currentValue) : sprintf(text, "%.2f", currentValue);
				m_lineEdit->SetValue(text);
				//m_container->Refresh();
				m_isEditing = false;
				});

			m_lineEdit->Bind(wxCUSTOMEVT_EDIT_FINISHED, [&](wxCommandEvent& event) {
				if (m_isEditing) return;
				m_isEditing = true;
				double newValue = m_prop->_value;
				wxString newValueStr = m_lineEdit->GetValue();
				bool success = newValueStr.ToDouble(&newValue);
				if (success) {
					newValue = std::clamp(newValue, m_prop->_range[0], m_prop->_range[1]);
					double currentValue = m_slider->getCurrentValue();
					m_slider->setCurrentValue(newValue);
					if (m_cb) {
						m_cb(currentValue);
					}
					m_container->Refresh();
		...
		void SliderBoxSizerWrapper::reset()
		{
			if (!m_prop || !m_slider || !m_lineEdit) return;
			m_slider->setCurrentValue(m_prop->_value);
			m_lineEdit->SetValue(m_prop->_textValue);
		}

		AnkerSlider* SliderBoxSizerWrapper::create_slider(wxPanel* panel, double value, const std::array<double, 2>& range, double step)
		{
			if (!panel || range.size() < 2) {
				return nullptr;
			}

			AnkerSlider* slider = new AnkerSlider(panel);
			slider->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
			slider->SetMinSize(AnkerSize(71, 20));
			slider->SetMaxSize(AnkerSize(71, 20));
			slider->setRange(range[0], range[1], step);
			slider->setCurrentValue(value);
			slider->setHandleRadius(3.0);
			slider->setTooltipVisible(false);
			return slider;
		}

		AnkerLineEditUnit* SliderBoxSizerWrapper::create_lineEditUnit(wxPanel* panel, const wxString& unit, const wxString& text, const std::array<double, 2>& range)
		{
			if (!panel || range.size() < 2) {
				return nullptr;
			}

			AnkerLineEditUnit* edit = new AnkerLineEditUnit(panel, unit, ANKER_FONT_NO_1, wxColour(41, 42, 45), wxColour("#3F4043"), 4, wxID_ANY);
			edit->SetFont(ANKER_FONT_NO_1);
			edit->SetMinSize(AnkerSize(80, 24));
			edit->SetMaxSize(AnkerSize(80, 24));
			edit->SetSize(AnkerSize(80, 24));
			edit->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
			edit->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT));
			edit->SetValue(text);
			m_bAngle == true ? edit->AddValidatorInt(range[0], range[1]) : edit->AddValidatorFloat(range[0], range[1], 2);
			return edit;
		}

		RadioBoxSizerWrapper::RadioBoxSizerWrapper(wxPanel* panel, const std::vector<wxString>& texts, std::function<void(size_t)> action, RadioBoxSizerType type, bool textLightColor)
		{
			if (!panel || texts.empty()) {
				return;
			}
			m_cb = action;
			m_type = type;
			if (type == RadioBoxSizerType_Horizontal) {
				m_boxSizer = new wxBoxSizer(wxHORIZONTAL);
			}
			else {
			...
			auto create_text = [&](const wxString& content, bool textLightColor = false) {
				auto text = new wxStaticText(panel, wxID_ANY, content, wxDefaultPosition, wxDefaultSize, wxNO_BORDER);
				text->SetFont(ANKER_FONT_NO_1);
				text->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
				textLightColor ? text->SetForegroundColour(wxColour(TEXT_LIGHT_RGB_INT)) : text->SetForegroundColour(wxColour(TEXT_DARK_RGB_INT));
				return text;
			};

			auto select = [&](wxButton* btn) {
				if (!btn) return;
				wxImage pressed_Image = wxImage(wxString::FromUTF8(Slic3r::var("brush_item_press.png")), wxBITMAP_TYPE_PNG);
				pressed_Image.Rescale(16, 16);
				btn->SetBitmap(pressed_Image);
				btn->SetName(BTN_PRESSED);
			};

			auto unselect = [&](wxButton* btn) {
				if (!btn) return;

				wxImage normal_Image = wxImage(wxString::FromUTF8(Slic3r::var("brush_item_normal.png")), wxBITMAP_TYPE_PNG);
				btn->SetBitmap(normal_Image);
				btn->SetName(BTN_NORMAL);
			};

			for (size_t i = 0; i < texts.size(); i++) {
				wxButton* btn = create_button(panel, BTN_NORMAL, wxString::FromUTF8(Slic3r::var("brush_item_normal.png")),
					wxString::FromUTF8(Slic3r::var("brush_item_press.png")));
				wxBoxSizer* buttonGroupSizer = new wxBoxSizer(wxHORIZONTAL);
				buttonGroupSizer->Add(btn, 1, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxALIGN_LEFT, 8);
				buttonGroupSizer->AddSpacer(5);
				wxStaticText* text = create_text(texts[i], textLightColor);
				buttonGroupSizer->Add(text, 0, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxALIGN_LEFT, 8);
				type == RadioBoxSizerType_Horizontal ? buttonGroupSizer->AddSpacer(5) : buttonGroupSizer->AddSpacer(15);

				m_boxSizer->Add(buttonGroupSizer, 1, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxALIGN_LEFT, 8);
				/*type == RadioBoxSizerType_Horizontal ? m_boxSizer->AddSpacer(5) : */m_boxSizer->AddSpacer(10);

				m_btns.push_back(RadioBoxButton(text, btn));
				btn->Bind(wxEVT_BUTTON, [&](wxCommandEvent& event) {
					wxButton* button = dynamic_cast<wxButton*>(event.GetEventObject());
					if (button) {
						if (button->GetName() == BTN_PRESSED)
							return;
						for (size_t j = 0; j < m_btns.size(); j++) {
							if (m_btns[j].second != nullptr && button != m_btns[j].second) {
								unselect(m_btns[j].second);
							}
							else if (m_btns[j].second != nullptr && button == m_btns[j].second) {
								select(m_btns[j].second);
								if (m_cb) {
									m_cb(j);
								}
							}
		...
		void RadioBoxSizerWrapper::select_item(size_t index)
		{
			if (index < m_btns.size()) {
				wxCommandEvent event(wxEVT_BUTTON, m_btns[index].second->GetId());
				event.SetEventObject(m_btns[index].second);
				wxPostEvent(m_btns[index].second->GetEventHandler(), event);
			}
		}

		void RadioBoxSizerWrapper::set_enable(bool enable)
		{
			//wxColour textColor = enable ? wxColour(PANEL_BACK_RGB_INT) : wxColour(TEXT_DARK_RGB_INT);
			for (size_t i = 0; i < m_btns.size(); i++) {
				//m_btns[i].first->Enable(enable);
				m_btns[i].second->Enable(enable);
				//m_btns[i].first->SetForegroundColour(textColor);
			}
		}

		wxButton* RadioBoxSizerWrapper::create_button(wxPanel* panel, const wxString& name, const wxString& normalImage, const wxString& pressImage)
		{
			wxImage normal_Image = wxImage(normalImage, wxBITMAP_TYPE_PNG);
			wxImage pressed_Image = wxImage(pressImage, wxBITMAP_TYPE_PNG);
			normal_Image.Rescale(16, 16);
			pressed_Image.Rescale(16, 16);
			auto btn = new wxButton(panel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxNO_BORDER);
			name == BTN_PRESSED ? btn->SetBitmap(pressed_Image) : btn->SetBitmap(normal_Image);
			btn->SetMinSize(normal_Image.GetSize());
			btn->SetMaxSize(normal_Image.GetSize());
			btn->SetSize(normal_Image.GetSize());
			btn->SetName(name);
			btn->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
