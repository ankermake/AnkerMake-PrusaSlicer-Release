...
#include "slic3r/GUI/PrinterConfigPlater/AnkerParameterData.hpp"
...
namespace Slic3r::GUI {
	...
	bool GLGizmoFdmSupports::on_init()
	{
		...
		// param
		m_desc["support_material"] = _L("support_material");
		m_desc["support_material_auto"] = _L("support_material_auto");
		m_desc["support_material_style"] = _L("support_material_style");
	...
	void GLGizmoFdmSupports::on_render_input_window(float x, float y, float bottom_limit)
	{
		...
		ModelConfigObject& obj_cfg = m_c->selection_info()->model_object()->config;
		bool supportEnable = obj_cfg.option("support_material")->getBool();
		bool supportAuto = obj_cfg.option("support_material_auto")->getBool();
		int supportThresholdAngle = obj_cfg.opt_int("support_material_threshold");
		Slic3r::SupportMaterialStyle supportStyle = Slic3r::SupportMaterialStyle(obj_cfg.opt_int("support_material_style"));

		int support_threshold_angle = supportAuto ? supportThresholdAngle : 0;
		...
		const float sliders_left_width = std::max(reset_button_slider_left, std::max(std::max(cursor_slider_left, clipping_slider_left), /*std::max(highlight_slider_left, gap_fill_slider_left)*/highlight_slider_left));
		const float slider_icon_width = m_imgui->get_slider_icon_size().x;
		const float max_tooltip_width = ImGui::GetFontSize() * 20.0f;
		...
		for (int i = 0; i < tool_ids.size() - 1; i++) {
		...
		if (ImGui::IsItemHovered()) {
			...
			if (m_support_threshold_angle != 0) {
				...
				m_imgui->text_colored(ImGuiWrapper::COL_SEPARATOR, str_tooltip);
			}
			else {
				wxString s_tooltip = (_L("No auto support"));
				m_imgui->text_colored(ImGuiWrapper::COL_SEPARATOR, s_tooltip);
			}
			ImGui::EndTooltip();
		}
		ImGui::SameLine(drag_left_width);
		ImGui::PushItemWidth(1.5 * slider_icon_width);
		if (ImGui::BBLDragFloat("##angle_threshold_deg_input", &m_highlight_by_angle_threshold_deg, 0.05f, 0.0f, 0.0f, "%.2f"))
		{
			m_highlight_by_angle_threshold_deg = std::max(0.0f, std::min(90.0f, m_highlight_by_angle_threshold_deg));
			m_parent.set_slope_normal_angle(90.f - m_highlight_by_angle_threshold_deg);
			if (!m_parent.is_using_slope()) {
				m_parent.use_slope(true);
				m_parent.set_as_dirty();
		...
		// support param settings
		bool warningFlag = false;
		bool seFlag = supportEnable;
		bool saFlag = supportAuto;
		auto def = obj_cfg.get().def();
		wxString seLabel = "";
		wxString saLabel = "";
		wxString ssLabel = "";
		if (def) {
			Slic3r::t_optiondef_map options = def->options;
			if (options.find("support_material") != options.end()) {
				seLabel = _L(options["support_material"].label);
			}
			if (options.find("support_material_auto") != options.end()) {
				saLabel = _L(options["support_material_auto"].label);
			}
			if (options.find("support_material_style") != options.end()) {
				ssLabel = _L(options["support_material_style"].label);
			}
		}
		...
		m_imgui->text(_L("Object Support Settings:"));
		m_imgui->bbl_checkbox(seLabel, seFlag);
		if (warningFlag)
		{
			ImGui::SameLine();
			m_imgui->text(wxString(ImGui::WarningMarkerSmall));
		}
		m_imgui->disabled_begin(!seFlag);
		m_imgui->bbl_checkbox(saLabel, saFlag);

		if (seFlag != supportEnable)
		{
			obj_cfg.set_key_value("support_material", new ConfigOptionBool{ seFlag });
			wxGetApp().sidebarnew().setObjectParamItemValue(_L("Support"), _L("support_material"), seFlag);
		}
		if (saFlag != supportAuto)
		{
			obj_cfg.set_key_value("support_material_auto", new ConfigOptionBool{ saFlag });
			wxGetApp().sidebarnew().setObjectParamItemValue(_L("Support"), _L("support_material_auto"), saFlag);
		}
		...
		{
			styleList.push_back(wxString(dataList.Item(i)->GetData()).utf8_string());
		}
		int comboSelection = wxGetApp().sidebarnew().GetModelParameterPanel()->getParameterData().m_ReVerStyleMap[supportStyle];
#ifdef __APPLE__
		double combo_text_len = 160.0;
		double combo_box_len = 150.0;
#else
		double combo_text_len = 80.0;
		double combo_box_len = 120.0;
#endif
		ImGui::AlignTextToFramePadding();
		const bool is_changed = m_imgui->combo(ssLabel, styleList, comboSelection, 0, combo_text_len, combo_box_len);
		if (is_changed)
		{
			Slic3r::SupportMaterialStyle newStyle = Slic3r::SupportMaterialStyle(wxGetApp().sidebarnew().GetModelParameterPanel()->getParameterData().m_StyleMap[comboSelection]);
			Slic3r::ConfigOptionEnum<Slic3r::SupportMaterialStyle>* configOption = new Slic3r::ConfigOptionEnum<Slic3r::SupportMaterialStyle>(newStyle);
			obj_cfg.set_key_value("support_material_style", configOption);
			wxGetApp().sidebarnew().setObjectParamItemValue(_L("Support"), _L("support_material_style"), comboSelection);
		}
		...
		if (warningFlag)
		{
			m_imgui->text(wxString(ImGui::WarningMarkerSmall) + _L("Enable_Support_Check_Tips"));
		}
