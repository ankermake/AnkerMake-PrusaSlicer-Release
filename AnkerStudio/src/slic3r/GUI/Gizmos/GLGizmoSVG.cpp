...
bool GLGizmoSVG::on_init()
{
    ...
    Point bed_size = wxGetApp().plater()->get_bed_shape().size();
    
    m_bed_size[0] = ((float)bed_size(0)) / 1000000.0f;
    m_bed_size[1] = ((float)bed_size(1)) / 1000000.0f;
    return true;
}

std::string GLGizmoSVG::on_get_name(bool i18n) const { return _u8L("SVG"); }
...
bool reset_button(const IconManager::Icons &icons)
{
    ...
    auto reset_icon = get_icon(icons, IconType::reset_value);
    auto reset_icon_hover = get_icon(icons, IconType::reset_value_hover);
    //reset_icon.size.x += 3;
    //reset_icon_hover.size.x += 3;
    return clickable(reset_icon, reset_icon_hover);
}
...
void GLGizmoSVG::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    //ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4.0 * screen_scale, 5.0 * screen_scale));
...
void GLGizmoSVG::draw_depth()
{
    ...
    if (ImGui::InputDouble("##depth", &input, step, step_fast, size_format)) {
        if (result_scale.has_value())
            input *= (*result_scale);
        if (input > step_fast * 100)     input = step_fast * 100;
        if (input < step)    input = step;
...
void GLGizmoSVG::draw_size() 
{
    ...
    if (m_keep_ratio) {
        ...
        if (m_imgui->slider_float_input("##width_size_slider", &width_f, minmax.min, minmax.max, ss.str().c_str(), 1.f, false)) {
            if (width_f > m_bed_size[0] * 100.0f) width_f = m_bed_size[0] * 100.0f;
            if(width_f < m_bed_size[0] / 100.0f) width_f = m_bed_size[0] / 100.0f;
        ...
        if (ImGui::InputDouble("##width", &width, step, fast_step, size_format, flags)) {
            if (width > m_bed_size[0] * 100.0f) width = m_bed_size[0] * 100.0f;
            if (width < m_bed_size[0] / 100.0f) width = m_bed_size[0] / 100.0f;
        ...
        if (ImGui::InputDouble("##height", &height, step, fast_step, size_format, flags)) {
            if (height > m_bed_size[1] * 100.0f)  height = m_bed_size[1] * 100.0f;
            if (height < m_bed_size[1] / 100.0f)   height = m_bed_size[1] / 100.0f;
...
void GLGizmoSVG::draw_distance()
{
    ...
    if (use_inch) {
        ...
        if (m_imgui->slider_optional_float_input("##distance", m_distance, min_distance, max_distance, "%.3f in", 1.f, false, move_tooltip)) {
        ...
        if (m_imgui->slider_optional_float_input("##distance", m_distance, min_distance, max_distance, "%.2f mm", 1.f, false, move_tooltip))
        {   
...
void GLGizmoSVG::draw_rotation()
{
    ...
    if (m_imgui->slider_float_input("##angle", &angle_deg, limits.angle.min, limits.angle.max, u8"%.2f °", 1.f, false, _L("Rotate text Clock-wise."))){
...
void GLGizmoSVG::draw_model_type()
{
    ...
    std::string cutName = _u8L("Cut");
    if (ImGui::RadioButton(_CTX_utf8(L_CONTEXT(cutName, "EmbossOperation"), "EmbossOperation").c_str(), type == negative))
        new_type = negative;
    else if (ImGui::IsItemHovered()) {
        if (is_last_solid_part)
    ...
    if (m_volume != nullptr && new_type.has_value() && !is_last_solid_part) {
        ...
        auto sel = obj_list->reorder_volumes_and_get_selection(
