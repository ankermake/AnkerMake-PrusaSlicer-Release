...
namespace {
    ...
    std::map<wxString, wxString> fontPaths;
    std::map<wxString, wxString> GetFontFilePaths();
    wxString GetSystemFontPath();
...
bool GLGizmoEmboss::on_init()
{
    ...
    checkFontStyleAvailability(m_style_manager);
    // Set rotation gizmo upwardrotate
    m_rotate_gizmo.set_angle(PI / 2);
    return true;
}

std::string GLGizmoEmboss::on_get_name(bool i18n) const { return _u8L("Emboss"); }
...
void GLGizmoEmboss::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * m_parent.get_scale());
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 3.0f * m_parent.get_scale());
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 2.0f * m_parent.get_scale());
    ...
    bool is_opened = true;
    ImGuiWindowFlags flag = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar;
    ...
    if (0/*m_should_set_minimal_windows_size*/) {
...
void GLGizmoEmboss::on_set_state()
{
    ...
    set_input_window_state(m_state);
}

std::string panelFlag = "GLGizmoEmboss";
void GLGizmoEmboss::set_input_window_state(bool on)
{
    return;
    if (wxGetApp().plater() == nullptr) {
        return;
    }
    ANKER_LOG_INFO << "GLGizmoEmboss::set_input_window_state: " << on;
    if (on) {
        initGUI();
    }
    else {
...
void GLGizmoEmboss::initGUI()
{
    ...
    if (m_pInputWindowSizer == nullptr)
    {
        ...
        container->setTitle(wxString::FromUTF8(get_name(true, false))/*_("engin_emboss")*/);
        ...
        container->Bind(wxANKEREVT_ATP_BUTTON_CLICKED, [this, returnBtnID](wxCommandEvent& event) {
            ...
            if (btnID == returnBtnID)
            {
                ...
                if (canvas) {
                    canvas->get_selection().clear();
                }
            }
            });

        m_pInputWindowSizer->Add(container, 1, wxEXPAND, 0);
        m_embossPanel = new wxPanel(container);
        m_embossPanel->SetBackgroundColour(wxColour(PANEL_BACK_RGB_INT));
        wxBoxSizer* embossPanelSizer = new wxBoxSizer(wxVERTICAL);
        m_embossPanel->SetSizer(embossPanelSizer);
        container->setContentPanel(m_embossPanel);

        // Embossed Text
        {
            wxString defaultEmbossedText = "Embossed Text";
            m_embossedTextEdit = new AnkerLineEditUnit(m_embossPanel, "", wxFont(ANKER_BOLD_FONT_BIG), wxColour(41, 42, 45), wxColour(TEXT_DARK_RGB_INT), 4);
            m_embossedTextEdit->SetValue(defaultEmbossedText);
            m_embossedTextEdit->SetMinSize(AnkerSize(392, 44));
            m_embossedTextEdit->SetMaxSize(AnkerSize(392, 44));
            m_embossedTextEdit->SetSize(AnkerSize(392, 44));
            m_embossedTextEdit->Bind(wxCUSTOMEVT_EDIT_FINISHED, [this](wxCommandEvent& event) {
                
                });

            //const auto& styles = m_style_manager.get_styles();

            embossPanelSizer->Add(m_embossedTextEdit, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
        ...
        // Style normal
        {
            ...
            embossPanelSizer->Add(styleSizer, 0, wxEXPAND | wxALIGN_TOP | wxLEFT | wxRIGHT, 10);
            //m_styleNormalComboBox = new 
...
void GLGizmoEmboss::checkFontStyleAvailability(Emboss::StyleManager& styleManger)
{
    std::map<wxString, wxString> defaultFontNames = GetFontFilePaths();
    StyleManager::Styles& styles = styleManger.get_styles();
    for (size_t i = 0; i < styles.size(); i++) {
        size_t pos = styles[i].path.find_last_of(';');
        if (pos != std::string::npos) {
            std::string fontName = styles[i].path.substr(pos + 1, styles[i].path.length() - 1);
#ifdef __APPLE__
            std::string xmlData = fontName;
            std::stringstream ss(xmlData);
            boost::property_tree::ptree pt;
            try {
                boost::property_tree::read_xml(ss, pt);
                fontName = pt.get<std::string>("plist.dict.string");
            }
...
#endif
            auto it = defaultFontNames.find(fontName);
            if (it != defaultFontNames.end()) {
                wxString ttfFile = it->second;
                if (boost::filesystem::exists(ttfFile.ToStdString().c_str())) {
                    ImGuiIO& io = ImGui::GetIO();
                    ImFontAtlas* atlas = io.Fonts;
                    if (atlas != nullptr && atlas->AddFontFromFileTTF(ttfFile.ToStdString().c_str(), 16.0f) != nullptr) {
                        continue;
                    }
                }
            }
...
void GLGizmoEmboss::draw_model_type()
{
    ...
    if (ImGui::RadioButton(_CTX_utf8(L_CONTEXT(_u8L("Cut"), "EmbossOperation"), "EmbossOperation").c_str(), type == negative))
        new_type = negative;
    else if (ImGui::IsItemHovered()) {
        if (is_last_solid_part)
...
void GLGizmoEmboss::draw_style_list() {
    ...
    if (ImGui::BBLBeginCombo("##style_selector", add_text_modify(trunc_name).c_str())) {
        ...
        for (const StyleManager::Style &style : styles) {
            ...
            ImVec2 select_size(max_style_name_width + m_gui_cfg->max_style_image_size.x(), select_height); // 0,0 --> calculate in draw
...
{    
    auto draw_slider_optional_int = [&]() -> bool {
        ...
        return m_imgui->slider_optional_int( ("##" + name).c_str(), value, 
            v_min, v_max, format.c_str(), 1.f, true, tooltip);
    };
    float undo_offset = ImGui::GetStyle().FramePadding.x;
...
void GLGizmoEmboss::draw_advanced()
{
    ...
    auto draw_align = [&align = font_prop.align, input_offset = m_gui_cfg->advanced_input_offset, &icons = m_icons]() {
        ...
        std::string alignment = _u8L("engin_emboss_alignment");
        std::string left = _u8L("Left");
        std::string center = _u8L("Center");
        std::string right = _u8L("Right");
        std::string top = _u8L("Top");
        std::string middle = _u8L("Middle");
        std::string bottom = _u8L("Bottom");
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(left, alignment), alignment.c_str()).c_str());
        ImGui::SameLine();
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(center, alignment), alignment.c_str()).c_str());
        ImGui::SameLine();
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(right, alignment), alignment.c_str()).c_str());
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(top, alignment), alignment.c_str()).c_str());
        ImGui::SameLine();
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(middle, alignment), alignment.c_str()).c_str());
        ImGui::SameLine();
        ...
        else if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", _CTX_utf8(L_CONTEXT(bottom, alignment), alignment.c_str()).c_str());
        return is_change;
    };
...
std::map<wxString, wxString> GetFontFilePaths() {
    if (!fontPaths.empty()) return fontPaths;
#ifdef _WIN32
    LOGFONT lf;
    ...
    EnumFontFamiliesEx(hdc, &lf, [](const LOGFONT* lpelfe, const TEXTMETRIC* lpntme, DWORD FontType, LPARAM lParam) -> int {
        wxString fontName = lpelfe->lfFaceName;
        LONG lRet;
        HKEY hKey;
        lRet = RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts", 0, KEY_READ, &hKey);
        if (lRet == ERROR_SUCCESS) {
            DWORD dwIndex = 0;
            DWORD dwType;
            WCHAR szValueName[MAX_PATH];
            BYTE szValueData[MAX_PATH * sizeof(WCHAR)];
            DWORD dwValueNameSize = MAX_PATH;
            DWORD dwValueDataSize = sizeof(szValueData);
            wxString sysFontPath = GetSystemFontPath();
            while (RegEnumValue(hKey, dwIndex, szValueName, &dwValueNameSize, NULL, &dwType, szValueData, &dwValueDataSize) == ERROR_SUCCESS) {
                wxString valueName(szValueName);
                wxString valueData(reinterpret_cast<wchar_t*>(szValueData), wxMBConvUTF16(), dwValueDataSize / sizeof(wchar_t));
                if (valueName.Lower().Find(fontName.Lower()) != wxNOT_FOUND) {
                    fontPaths[fontName] = sysFontPath + valueData;
                    break;
                }
        ...
        return 1;
        }, (LPARAM)NULL, (DWORD)NULL);
    ReleaseDC(NULL, hdc);
#elif __APPLE__
    const char* fontDirectories[] = {
            "/System/Library/Fonts",
            "/Library/Fonts",
            "~/Library/Fonts"
        };

        for (const char* directory : fontDirectories) {
                ...
                while ((entry = readdir(dir)) != nullptr) {
                    ...
                    if (stat(filePath.c_str(), &fileInfo) == 0) {
                        if (S_ISREG(fileInfo.st_mode)) {
                            // Assuming the font name is the file name without the extension
                            std::string fontName = entry->d_name;
...
#ifdef _WIN32
    PWSTR pPath = NULL;
    HRESULT result = SHGetKnownFolderPath(FOLDERID_Fonts, 0, NULL, &pPath);
    if (result == S_OK) {
...
void init_face_names(Facenames &face_names)
{
    ...
    // validation lambda
    auto is_valid_font = [encoding = face_names.encoding, bad = face_names.bad /*copy*/](const wxString &name, const std::map<wxString, wxString>& defaultFontNames) {
        ...
        if(name.Lower() == ("symbol") ||
           name.Lower() == ("apple braille"/*Braille character*/) ||
           name.Lower().Contains("emoji") ||
            name.Lower().Contains("icons") ||
            name.Lower() == ("sans serif collection"/*A collection of typefaces*/) ||
            name.Lower() == ("segoe mdl2 assets")/*Collection of ICONS and symbols*/ ||
            name.Lower() == ("webdings")/*symbol font*/ ||
            name.Lower() == ("wingdings")/*symbol font*/) return false;
        if(defaultFontNames.find(name) == defaultFontNames.end())    return false;
        wxString ttfFile = defaultFontNames.find(name)->second;
        if (!boost::filesystem::exists(ttfFile.ToStdString().c_str())) return false;
        ImGuiIO& io = ImGui::GetIO();
        ImFontAtlas* atlas = io.Fonts;
        if (atlas == nullptr) return false;
        if (atlas->AddFontFromFileTTF(ttfFile.ToStdString().c_str(), 16.0f) == nullptr) return false;
    ...
    for (const wxString &name : facenames) {
        if (is_valid_font(name, defaultFontNames)) {
...
GuiCfg create_gui_configuration()
{
    ...
    tr.alignment = _u8L("engin_emboss_alignment");
    // TRN - Input label. Be short as possible
