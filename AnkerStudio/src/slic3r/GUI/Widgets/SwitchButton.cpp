...
void SwitchButton::Rescale()
{
	if (labels[0].IsEmpty()) {
...
#else
        constexpr int BS = 1;
		float scale = Slic3r::GUI::wxGetApp().em_unit() / 10.f;
		if (scale == 1.2) {
			scale = 1.25f;
		}
#endif
		...
		for (int i = 0; i < 2; ++i) {
			...
			memdc.Clear();
			memdc.SetFont(dc.GetFont().Scaled(scale));
#else
			...
			auto state = i == 0 ? StateColor::Enabled : (StateColor::Checked | StateColor::Enabled);
            {
                ...
                wxDC &dc2(memdc);
#endif
				dc2.SetBrush(wxBrush(track_color.colorForStatesDark(state)));
				dc2.SetPen(wxPen(track_color.colorForStatesDark(state)));
                dc2.DrawRoundedRectangle(wxRect({0, 0}, trackSize), trackSize.y / 2);
				dc2.SetBrush(wxBrush(thumb_color.colorForStatesDark(StateColor::Checked | StateColor::Enabled)));
				dc2.SetPen(wxPen(thumb_color.colorForStatesDark(StateColor::Checked | StateColor::Enabled)));
				dc2.DrawRoundedRectangle(wxRect({ i == 0 ? BS : (trackSize.x - thumbSize.x - BS), BS}, thumbSize), thumbSize.y / 2);
			}
            memdc.SetTextForeground(text_color.colorForStatesDark(state ^ StateColor::Checked));
            memdc.DrawText(labels[0], {BS + (thumbSize.x - textSize[0].x) / 2, BS + (thumbSize.y - textSize[0].y) / 2});
            memdc.SetTextForeground(text_color2.count() == 0 ? text_color.colorForStatesDark(state) : text_color2.colorForStatesDark(state));
