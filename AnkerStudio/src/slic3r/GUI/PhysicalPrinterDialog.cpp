...
void PhysicalPrinterDialog::update(bool printer_change)
{
    ...
    if (tech == ptFFF) {
        ...
        if (opt->value == htAnkerLink) { // AnkerConnect does NOT allow http digest
            ...
            if (opt->value == htAnkerConnect) { // automatically show default Ankerconnect address
                if (Field* printhost_field = m_optgroup->get_field("print_host"); printhost_field) {
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
    ...
    auto model_supports_ankerlink = [](const std::string& model) {
        return model.size() >= 2 &&
    ...
    auto model_supports_ankerconnect = [](const std::string& model) {
        return model.size() >= 2 &&
                ((boost::starts_with(model, "MK") && model[2] > '2' && model[2] <= '9')
                || boost::starts_with(model, "MK2.5")
                || boost::starts_with(model, "XL")
    ...
    // set all_presets_are_ankerlink_supported
    for (PresetForPrinter* prstft : m_presets) {
        ...
        if (Preset* preset = wxGetApp().preset_bundle->printers.find_preset(preset_name)) {
            ...
            if (preset->vendor) {
                if (preset->vendor->name == "Anker Research") {
                    ...
                    if (it != models.end() && model_supports_ankerlink(it->family))
                        continue;
                }
            }
            else if (model_supports_ankerlink(model_id))
                continue;
        }
    ...
    // set all_presets_are_ankerconnect_supported
    for (PresetForPrinter* prstft : m_presets) {
        std::string preset_name = prstft->get_preset_name();
        ...
        std::string model_id = preset->config.opt_string("printer_model");
        if (preset->vendor && preset->vendor->name != "Anker Research") {
            connect.supported = false;
            break;
        }
        if (preset->vendor && preset->vendor->name != "Anker Research") {
        ...
        if (!model_supports_ankerconnect(model_id)) {
            connect.supported = false;
            break;
        }
    }
    ...
    if (link.supported && link.label == _(ht->m_opt.enum_def->label(index_in_choice)))
        m_config->set_key_value("host_type", new ConfigOptionEnum<PrintHostType>(htAnkerLink));
    else if (connect.supported && connect.label == _(ht->m_opt.enum_def->label(index_in_choice)))
        m_config->set_key_value("host_type", new ConfigOptionEnum<PrintHostType>(htAnkerConnect));
    else {
