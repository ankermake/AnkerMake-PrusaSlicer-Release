#ifndef slic3r_AnkerOptionsGroup_hpp_
#define slic3r_AnkerOptionsGroup_hpp_
...
class AnkerUIBuildCanceled : public std::exception {};
...
struct AnkerOption {
	...
	bool operator==(const AnkerOption& rhs) const {
		return  (rhs.opt_id == this->opt_id);
	}

	AnkerOption(const ConfigOptionDef& _opt, t_config_option_key id);
};
using t_AnkerOption = std::unique_ptr<AnkerOption>;	//!

/// Represents option lines
class AnkerLine : public UndoValueUIManager
{
...
public:
    ...
    void append_option(const AnkerOption& option) {
        m_options.push_back(option);
    }
	void append_widget(const widget_t widget) {
		m_extra_widgets.push_back(widget);
    }
	AnkerLine(wxString label, wxString tooltip) :
		label(_(label)), label_tooltip(_(tooltip)) {}
	AnkerLine() : m_is_separator(true) {}

	AnkerLine(const std::string& opt_key, const wxString& label, const wxString& tooltip) :
		label(_(label)), label_tooltip(_(tooltip))
	{
		m_options.push_back(AnkerOption({ opt_key, coNone }, opt_key));
	}
    ...
    const std::vector<AnkerOption>&		get_options() const { return m_options; }

private:
	std::vector<AnkerOption>		m_options;//! {std::vector<AnkerOption>()};
    std::vector<widget_t>	m_extra_widgets;//! {std::vector<widget_t>()};
};

using AnkerColumn_t = std::function<wxWindow*(wxWindow* parent, const AnkerLine&)>;

using t_optionfield_map = std::map<t_config_option_key, t_field>;
using t_opt_map = std::map< std::string, std::pair<std::string, int> >;

class AnkerOptionsGroup {
protected:
	wxStaticBox*	stb {nullptr};
public:
    bool		staticbox {true};
    const wxString	title;
    size_t			label_width = 30 ;// {200};
    wxSizer*		sizer {nullptr};
	wxBoxSizer*		groupHeaderSizer{nullptr};
	Anker_OG_CustomCtrl*  custom_ctrl{ nullptr };
	int				ctrl_horiz_alignment{ /*wxALIGN_LEFT*/wxALIGN_RIGHT};
	AnkerColumn_t		extra_column {nullptr};
	...
	void		append_line(const AnkerLine& line);
	// create controls for the option group
	void		activate_line(AnkerLine& line);

	// modify by dhf for anker optionGroup line style
	// create all controls for the option group from the m_lines
	bool		activate(std::function<void()> throw_if_canceled = [](){}, int horiz_alignment = /*wxALIGN_LEFT*/wxALIGN_RIGHT);
	// delete all controls from the option group
	void		clear(bool destroy_custom_ctrl = false);

	AnkerLine		create_single_option_line(const AnkerOption& option, const std::string& path = std::string()) const;
    void		append_single_option_line(const AnkerOption& option, const std::string& path = std::string()) { append_line(create_single_option_line(option, path)); }
    ...
    inline AnkerLine*	get_line(const t_config_option_key& id) {
	...
	AnkerOptionsGroup(	wxWindow* _parent, const wxString& title, bool is_tab_opt = false,
		AnkerColumn_t extra_clmn = nullptr);
	// add by dhf for setting Anker style (no static_box) OptionsGroup 
	AnkerOptionsGroup(	wxWindow* _parent, const wxString& title, bool is_tab_opt = false,
		bool use_static_box = false, AnkerColumn_t extra_clmn = nullptr);
	virtual ~AnkerOptionsGroup() { clear(true); }

    wxGridSizer*        get_grid_sizer() { return m_grid_sizer; }
	const std::vector<AnkerLine>& get_lines() { return m_lines; }
	bool				is_legend_line();
	...
	const std::map<t_config_option_key, AnkerOption>& get_optioms_map() { return m_options; }

	bool				is_activated() { return sizer != nullptr; }
	wxString 			get_OptionGroup_icon_name(const wxString& groupName);
protected:
	std::map<t_config_option_key, AnkerOption>	m_options;
    wxWindow*				m_parent {nullptr};
    std::vector<ConfigOptionMode>           m_options_mode;
    std::vector<wxWindow*>                  m_extra_column_item_ptrs;

    std::vector<AnkerLine>                       m_lines;
	...
	const t_field&		build_field(const AnkerOption& opt);
...
public:
	...
	static bool			sidetext_is_unit(wxString sideText);
};

class AnkerConfigOptionsGroup: public AnkerOptionsGroup {
public:
	AnkerConfigOptionsGroup(	wxWindow* parent, const wxString& title, DynamicPrintConfig* config = nullptr,
						bool is_tab_opt = false, AnkerColumn_t extra_clmn = nullptr) :
		AnkerOptionsGroup(parent, title, is_tab_opt, extra_clmn), m_config(config) {}
	// add by dhf for setting Anker style (no static_box) OptionsGroup 
	AnkerConfigOptionsGroup(wxWindow* parent, const wxString& title, DynamicPrintConfig* config = nullptr,
		bool is_tab_opt = false, AnkerColumn_t extra_clmn = nullptr, bool use_static_box = false) :
		AnkerOptionsGroup(parent, title, is_tab_opt, use_static_box, extra_clmn), m_config(config) {}
	AnkerConfigOptionsGroup(	wxWindow* parent, const wxString& title, ModelConfig* config,
						bool is_tab_opt = false, AnkerColumn_t extra_clmn = nullptr) :
		AnkerOptionsGroup(parent, title, is_tab_opt, extra_clmn), m_config(&config->get()), m_modelconfig(config) {}
	AnkerConfigOptionsGroup(	wxWindow* parent) :
		AnkerOptionsGroup(parent, wxEmptyString, true, nullptr) {}
    ~AnkerConfigOptionsGroup() override = default;
	...
	AnkerOption		get_option(const std::string& opt_key, int opt_index = -1);
	AnkerLine		create_single_option_line(const std::string& title, const std::string& path = std::string(), int idx = -1) /*const*/{
		AnkerOption option = get_option(title, idx);
		return AnkerOptionsGroup::create_single_option_line(option, path);
	}
	AnkerLine		create_single_option_line(const AnkerOption& option, const std::string& path = std::string()) const {
		return AnkerOptionsGroup::create_single_option_line(option, path);
	}
	void		append_single_option_line(const AnkerOption& option, const std::string& path = std::string())	{
		AnkerOptionsGroup::append_single_option_line(option, path);
	}
	void		append_single_option_line(const std::string title, const std::string& path = std::string(), int idx = -1)
	{
		AnkerOption option = get_option(title, idx);
		if (m_opt_width > 0)
			option.opt.width = m_opt_width;
	...
	void        set_opt_width(int w) { m_opt_width = w; };
	int         get_opt_width() { return m_opt_width; };
private:
...
class AnkerogStaticText :public wxStaticText{
public:
	AnkerogStaticText() {}
	AnkerogStaticText(wxWindow* parent, const wxString& text);
	~AnkerogStaticText() {}
