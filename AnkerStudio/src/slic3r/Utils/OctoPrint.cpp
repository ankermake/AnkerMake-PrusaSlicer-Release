...
bool OctoPrint::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    if (resolved_addr.empty()) {
        ...
        BOOST_LOG_TRIVIAL(error) << "AnkerStudio failed to resolve hostname " << m_host << " into the IP address. Starting upload with system resolving.";
        return upload_inner_with_host(std::move(upload_data), prorgess_fn, error_fn, info_fn);
...
{
    return _(L("Connection to Anker SL1 / SL1S works correctly."));
}

wxString SL1Host::get_test_failed_msg (wxString &msg) const
{
    return GUI::format_wxstr("%s: %s", _L("Could not connect to Anker SLA"), msg);
}

bool SL1Host::validate_version_text(const boost::optional<std::string> &version_text) const
{
    return version_text ? boost::starts_with(*version_text, "Anker SLA") : false;
}

// AnkerLink
AnkerLink::AnkerLink(DynamicPrintConfig* config, bool show_after_message) :
    OctoPrint(config),
...
const char* AnkerLink::get_name() const { return "AnkerLink"; }

wxString AnkerLink::get_test_ok_msg() const
{
    return _(L("Connection to AnkerLink works correctly."));
}

wxString AnkerLink::get_test_failed_msg(wxString& msg) const
{
    return GUI::format_wxstr("%s: %s", _L("Could not connect to AnkerLink"), msg);
}

bool AnkerLink::validate_version_text(const boost::optional<std::string>& version_text) const
{
    return version_text ? (boost::starts_with(*version_text, "AnkerLink") || boost::starts_with(*version_text, "OctoPrint")) : false;
}

void AnkerLink::set_auth(Http& http) const
{
...
#if 0
bool AnkerLink::version_check(const boost::optional<std::string>& version_text) const
{
    ...
    try {
        ...
        if (name_and_version.front() == "AnkerLink" && semver >= Semver(0, 7, 0))
            return true;
...
bool AnkerLink::test(wxString& msg) const
{
    // Since the request is performed synchronously here,
    // it is ok to refer to `msg` from within the closure
    const char* name = get_name();
...
bool AnkerLink::get_storage(wxArrayString& storage_path, wxArrayString& storage_name) const
{
    const char* name = get_name();
    ...
    .on_complete([&](std::string body, unsigned) {
        ...
        try
        {
            ...
            for (const auto& section : ptree.front().second) {
                ...
                const auto ro = section.second.get_optional<bool>("ro"); // In AnkerLink 0.7.0RC2 "read_only" value is stored under "ro".
...
bool AnkerLink::test_with_method_check(wxString& msg, bool& use_put) const
{
    // Since the request is performed synchronously here,
    // it is ok to refer to `msg` from within the closure
...
#ifdef WIN32
bool AnkerLink::test_with_resolved_ip_and_method_check(wxString& msg, bool& use_put) const
{
    // Since the request is performed synchronously here,
    // it is ok to refer to `msg` from within the closure
    const char* name = get_name();
...
bool AnkerLink::upload_inner_with_resolved_ip(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn, const boost::asio::ip::address& resolved_addr) const
{
    info_fn(L"resolve", boost::nowide::widen(resolved_addr.to_string()));
...
bool AnkerLink::upload_inner_with_host(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    const char* name = get_name();
...
bool AnkerLink::put_inner(PrintHostUpload upload_data, std::string url, const std::string& name, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    set_auth(http);
    // This is ugly, but works. There was an error at AnkerLink side that accepts any string at Print-After-Upload as true, thus False was also triggering print after upload.
...
bool AnkerLink::post_inner(PrintHostUpload upload_data, std::string url, const std::string& name, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    info_fn(L"set_complete_off", wxString());
    bool res = true;
    const auto upload_filename = upload_data.upload_path.filename();
...
void AnkerLink::set_http_post_header_args(Http& http, PrintHostPostUploadAction post_action) const
{
    http.form_add("print", post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false");
}

AnkerConnect::AnkerConnect(DynamicPrintConfig* config)
    : AnkerLink(config, true)
{
}

void AnkerConnect::set_http_post_header_args(Http& http, PrintHostPostUploadAction post_action) const
{
...
wxString AnkerConnect::get_test_ok_msg() const
{
    return _(L("Connection to AnkerConnect works correctly."));
}

wxString AnkerConnect::get_test_failed_msg(wxString& msg) const
{
    return GUI::format_wxstr("%s: %s", _L("Could not connect to AnkerConnect"), msg);
}
