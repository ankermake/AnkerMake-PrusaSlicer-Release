...
class AnkerLink : public OctoPrint
{
public:
    AnkerLink(DynamicPrintConfig* config) : AnkerLink(config, false) {}
    AnkerLink(DynamicPrintConfig* config, bool show_after_message);
    ~AnkerLink() override = default;
...
class AnkerConnect : public AnkerLink
{
public:
    AnkerConnect(DynamicPrintConfig* config);
    ~AnkerConnect() override = default;
    wxString get_test_ok_msg() const override;
    wxString get_test_failed_msg(wxString& msg) const override;
    ...
    const char* get_name() const override { return "AnkerConnect"; }
...
class SL1Host : public AnkerLink
{
public:
