...
int CLI::run(int argc, char **argv)
{
    ...
    bool 							start_as_gcodeviewer =
            ...
            false;
#else
            // On Unix systems, the ankermake_alpha binary may be symlinked to give the application a different meaning.
...
void boostLogInit() {
    ...
    logPath += "/log/AnkerMake_Alpha_%Y-%m-%d_%H-%M-%S.%N.log";
    ...
    auto fileSink = logging::add_file_log(
        ...
        keywords::rotation_size = 100 * 1024 * 1024,  // Archive every 50M and generate a log file
    ...
    logging::formatter formatter =
        ...
        << expressions::format_date_time<boost::posix_time::ptime>("TimeStamp",
            "%Y-%m-%d %H:%M:%S") /*.%f*/
        << "|"
        << expressions::attr<logging::attributes::current_thread_id::value_type>(
            "ThreadID")
        << "]["
        << expressions::attr<logging::trivial::severity_level>("Severity")
        << "]"
    ...
    ANKER_LOG_INFO << "The log path is " << logPath.c_str();
}
...
bool CLI::setup(int argc, char **argv)
{
    ...
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the AnkerMake_alpha process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"AnkerMake_alpha is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using AnkerMake_alpha.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes AnkerMake_alpha "
                L"to crash on a secondary monitor, see AnkerMake_alpha github issue #5573";
        MessageBoxW(NULL, text.c_str(), L"Warning"/*L"Incopatible library found"*/, MB_OK);
    }
#endif

    // See Invoking ankermake_alpha from $PATH environment variable crashes #5542
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    boost::nowide::cout
        ...
        << "Usage: ankermake_alpha [ ACTIONS ] [ TRANSFORM ] [ OPTIONS ] [ file.stl ... ]" << std::endl
        << std::endl
        << "Actions:" << std::endl;
