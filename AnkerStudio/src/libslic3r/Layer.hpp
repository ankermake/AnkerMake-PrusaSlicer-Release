...
public:
    ...
    [[nodiscard]] const SurfaceCollection&          slices() const  { return m_slices; }
                               inline void          update_slices(const ExPolygons& src, SurfaceType surfaceType) { return m_slices.set(src, surfaceType); }
                               inline void          update_slices(const ExPolygons&& src, SurfaceType surfaceType) { return m_slices.set(src, surfaceType); }

    // Unspecified fill polygons, used for overhang detection ("ensure vertical wall thickness feature")
    // and for re-starting of infill.
    ...
    void    make_perimeters(
        ...
        // Ranges of perimeter extrusions and gap fill extrusions per surface, referencing
    ...
    // Single elephant foot compensation step, used by the elephant foot compensation at the 1st layer.
    ...
    //BBS
    void    simplify_infill_extrusion_entity() { simplify_entity_collection(&m_fills); }
    void    simplify_wall_extrusion_entity() { simplify_entity_collection(&m_perimeters); }
private:
...
public:
    ...
    // Infill + gap_fill extrusions in LayerRegion belonging to this island.
...
struct LayerSlice
{
    ...
    //add by friva,get the perimeters  regions id of this layer slice(islands)
    std::set<uint32_t>   perimeters_region_ids() const {
        std::set<uint32_t> _perimeters_region_ids;
        for (const LayerIsland& island : islands) {
			_perimeters_region_ids.insert(island.perimeters.region());
		}
	return _perimeters_region_ids;
    };
...
//    bool                slicing_errors;
    ...
    // For the first layer, if the Elephant foot compensation is applied, this slices is uncompensated, therefore
    // it includes the Elephant foot effect, thus it corresponds to the shape of the printed 1st layer.
    // These slices aka islands are chained by the shortest traverse distance and this traversal
    // order will be applied by the G-code generator to the extrusions fitting into these slices.
    // These slices are also used to detect overhangs and overlaps between successive layers, therefore it is important
    // that the 1st slices is not compensated by the Elephant foot compensation algorithm.
    ExPolygons 				lslices;
    ...
    void                    export_lslices_to_svg(const char* path) const;
    void                    export_region_slices_to_svg(const char *path) const;
    void                    export_region_fill_surfaces_to_svg(const char *path) const;
    // Export to "out/LayerRegion-name-%d.svg" with an increasing index with every export.
    void                    export_region_slices_to_svg_debug(const char *name) const;
...
private:
    ...
    // Sequential index of layer, 0-based, offset by number of raft layers.
    size_t              m_id;
