...
{
    if (object == nullptr || src_volume == nullptr) {
        return;
    }
    if (mesh.empty())
        return;
                                 ...
                                 ModelObjectCutAttributes attributes, ModelObject* upper, ModelObject* lower)
{
    ...
    if (attributes.has(ModelObjectCutAttribute::KeepAsParts)) {
        if (upper) upper->add_volume(*volume);
        return;
    }
    ...
    if (attributes.has(ModelObjectCutAttribute::KeepLower) && (bb.max[Z] <= 0 || is_crossed_by_cut))
        if(lower) lower->add_volume(*volume);
}
                            ...
                            ModelObjectCutAttributes attributes, ModelObject* upper, ModelObject* lower)
{
    ...
    if (attributes.has(ModelObjectCutAttribute::KeepAsParts) && upper) {
    ...
    if (attributes.has(ModelObjectCutAttribute::KeepUpper) && upper)
        add_cut_volume(upper_mesh, upper, volume, cut_matrix);

    if (attributes.has(ModelObjectCutAttribute::KeepLower) && lower && !lower_mesh.empty())
...
const ModelObjectPtrs& Cut::perform_with_plane()
{
    ...
    if (m_attributes.has(ModelObjectCutAttribute::KeepAsParts) && upper && upper->volumes.empty()) {
        m_model = Model();
