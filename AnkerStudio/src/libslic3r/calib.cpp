...
float CalibPressureAdvance::find_optimal_PA_speed(const DynamicPrintConfig &config, double line_width, double layer_height, int filament_idx)
{
    ...
    auto         pa_speed = std::max(general_suggested_min_speed, config.get_abs_value("external_perimeter_speed"));
    if (filament_max_volumetric_speed > 0)
    {
        pa_speed = std::min(pa_speed, filament_max_volumetric_speed / pattern_line.mm3_per_mm());
    }

    return std::floor(pa_speed);
}

std::string CalibPressureAdvance::move_to(Vec2d pt, GCodeWriter &writer, std::string comment, double zhop_height, double layer_height)
{
    std::stringstream gcode;

    Vec2d a2b = Vec2d(m_last_pos.x(), m_last_pos.y()) - pt;
    double dist = a2b.norm();
    if (dist > 1.5) {
    ...
    if (zhop_height > 0) {
        ...
        gcode << writer.travel_to_z(layer_height, "layer change");
    }

    gcode << writer.travel_to_xy(pt, comment);

    if (zhop_height > 0 && layer_height > 0) {
        gcode << writer.travel_to_z(layer_height, "undo z-hop");
    }
    ...
    GCodeWriter &writer, Vec2d to_pt, double line_width, double layer_height, double speed, const std::string &comment)
{
    const double e_per_mm = CalibPressureAdvance::e_per_mm(line_width, layer_height,
                                                           ...
                                                           1/*m_config.option<ConfigOptionFloats>("filament_flow_ratio")->get_at(0)*/);
...
bool CalibPressureAdvanceLine::is_delta() const { return mp_gcodegen->config().bed_shape.values.size() > 4; }
                                                         ...
                                                         const Vec3d              &origin)
{
    ...
    for (int i = 0; i < m_num_layers; ++i) {
        ...
        const double zhop_height = -1;// layer_height + height_layer();
        ...
        if (i == 1) {
            ...
            double number_e_per_mm = e_per_mm(line_width(), height_layer(),
                                              m_config.option<ConfigOptionFloats>("nozzle_diameter")->get_at(0),
                                              m_config.option<ConfigOptionFloats>("filament_diameter")->get_at(0),
                                              1/*m_config.option<ConfigOptionFloats>("print_flow_ratio")->get_at(0)*/);
        ...
        gcode << move_to(Vec2d(to_x, to_y), m_writer, "Move to pattern start",-1.0, layer_height);
        ...
        for (int j = 0; j < num_patterns; ++j) {
            ...
            for (int k = 0; k < wall_count(); ++k) {
                ...
                if (k != wall_count() - 1) {
                    ...
                    gcode << move_to(Vec2d(to_x, to_y), m_writer, "Move to start next pattern wall", zhop_height, zhop_height > 0 ? layer_height : -1.);
                } else if (j != num_patterns - 1) {
                    // patterns not done yet. move to next pattern
                    to_x += m_pattern_spacing + line_width();
                    gcode << move_to(Vec2d(to_x, to_y), m_writer, "Move to next pattern", zhop_height, zhop_height > 0 ? layer_height : -1.);
                } else if (i != m_num_layers - 1) {
                    // layers not done yet. move back to start
                    to_x = initial_x;
                    gcode << move_to(Vec2d(to_x, to_y), m_writer, "Move back to start position", zhop_height, zhop_height > 0 ? layer_height : -1.);
                } else {
                    // everything done
                }
            }
                                                ...
                                                const Vec3d              &origin)
{
    ...
    _refresh_starting_point(model, origin);
    _refresh_writer(is_bbl_machine, model, origin);
}

void CalibPressureAdvancePattern::_refresh_starting_point(const Model &model, const Vec3d& origin)
{
    if (m_is_start_point_fixed)
        return;

    ModelObject  *obj  = model.objects.front();
    BoundingBoxf3 bbox = obj->instance_bounding_box(0, false);

    m_starting_point = Vec3d(bbox.min.x(), bbox.max.y(), 0) + origin;
