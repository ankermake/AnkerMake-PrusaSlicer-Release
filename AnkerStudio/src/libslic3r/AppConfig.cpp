...
void AppConfig::set_defaults()
{
    if (m_mode == EAppMode::Editor) {
        ...
        // By default, Anker has the controller hidden.
...
std::string AppConfig::load(const std::string &path)
{
    ...
    try {
        ...
        if (!recovered) {
            // Report the initial error of parsing AnkerStudio.ini.
...
void AppConfig::save()
{
    ...
    // On Windows, the rename is likely NOT atomic, thus it may fail if AnkerStudio crashes on another thread in the meanwhile.
