...
unsigned char FromHex(unsigned char x)
{
	...
	if (x >= 'A' && x <= 'Z') y = x - 'A' + 10;
		...
		ANKER_LOG_ERROR<<"from hex error with: "<<x;
	return y;
}
...
{
	return std::string("generated by AnkerMake Studio " SLIC3R_VERSION " on " ) + Utils::utc_timestamp();
}
...
boost::filesystem::path getLogDirPath() {
	...
	boost::filesystem::path logPath = executablePath.parent_path();
