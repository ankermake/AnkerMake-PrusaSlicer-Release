...
protected:
    ...
    std::string move_to(Vec2d pt, GCodeWriter &writer, std::string comment = std::string(), double zhop_height = -1.0, double layer_height = -1.0);
...
{
    const std::vector<std::pair<std::string, double>> float_or_percent_pairs{{"first_layer_height", 0.25},
                                                                  {"first_layer_speed", 30}};

    const std::vector<std::pair<std::string, double>> nozzle_ratio_pairs{{"extrusion_width", 112.5}, {"first_layer_extrusion_width", 140}};

    const std::vector<std::pair<std::string, int>> int_pairs{{"skirts", 0}, {"perimeters", 3}};
...
protected:
    double speed_first_layer() const { return m_config.option<ConfigOptionFloatOrPercent>("first_layer_speed")->value; };
    double speed_perimeter() const { return m_config.get_abs_value("external_perimeter_speed", false); };
    double line_width_first_layer() const { return m_config.get_abs_value("first_layer_extrusion_width"); };
    double line_width() const { return m_config.get_abs_value("extrusion_width", false); };
    int    wall_count() const { return m_config.option<ConfigOptionInt>("perimeters")->value; };

private:
    void refresh_setup(const DynamicPrintConfig &config, bool is_bbl_machine, const Model &model, const Vec3d &origin);
    void _refresh_starting_point(const Model &model, const Vec3d& origin);
    void _refresh_writer(bool is_bbl_machine, const Model &model, const Vec3d &origin);

    double    height_first_layer() const { return m_config.get_abs_value("first_layer_height",false); };
