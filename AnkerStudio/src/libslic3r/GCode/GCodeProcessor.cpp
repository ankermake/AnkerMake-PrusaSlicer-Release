...
static const float DEFAULT_ACCELERATION = 1500.0f; // Anker Firmware 1_75mm_MK2
static const float DEFAULT_RETRACT_ACCELERATION = 1500.0f; // Anker Firmware 1_75mm_MK2
...
// taken from PrusaResearch.ini - [printer:Original Anker i3 MK2.5 MMU2]
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    { EProducer::AnkerStudio, "generated by AnkerMake Studio" },
    { EProducer::PrusaSlicer, "generated by PrusaSlicer" },
    { EProducer::Slic3rPE,    "generated by Slic3r Anker Edition" },
...
void GCodeProcessor::apply_config(const PrintConfig& config)
{
    ...
    m_producer = EProducer::AnkerStudio;
    ...
    // With MM setups like Anker MMU2, the filaments may be expected to be parked at the beginning.
...
void GCodeProcessor::apply_config(const DynamicPrintConfig& config)
{
    ...
    const ConfigOptionStrings* extruder_colour = config.option<ConfigOptionStrings>("filament_colour");
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by AnkerStudio,
        // extract the config from it
        if (m_producer == EProducer::AnkerStudio || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from AnkerStudio's own G-code.
...
bool GCodeProcessor::isRecognizedProducer() const
{
    return (m_producer == EProducer::AnkerStudio || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r);
}

void GCodeProcessor::process_file_thumbnail_only(const std::string& filename, const Vec2d& encode_sizes, std::string& format_str, std::string& out_base64_string, std::function<void()> cancel_callback)
{
    CNumericLocalesSetter locales_setter;

    bool has_searched_tag = false;
    m_parser.parse_file_raw(filename, [this ,&has_searched_tag ,&encode_sizes ,&format_str ,&out_base64_string](GCodeReader& reader, const char* begin, const char* end) {
        begin = skip_whitespaces(begin, end);
        if (begin != end && *begin == ';') {
            // Comment.
            begin = skip_whitespaces(++begin, end);
            ...
            if (begin != end && detect_thumbnail_end(std::string_view(begin, end - begin), format_str, has_searched_tag))
                m_parser.quit_parsing();

            if (begin != end && has_searched_tag) {
                out_base64_string.append(begin, end);
            }

            if (begin != end && search_for_thumbnail_target(std::string_view(begin, end - begin), encode_sizes, format_str, 0))
                has_searched_tag = true;
        }
        });
    m_parser.reset();
}

void GCodeProcessor::process_file_ext(const std::string& filename, GCodeProcessorResultExt& out)
{
    CNumericLocalesSetter locales_setter;
    ...
    if (isAcodeFile(filename)){
        //acode file set the defaut  producer use AnkerMake
        has_detect_producer = true;
        m_producer = EProducer::AnkerMake;
    };
    ...
    m_parser.parse_file_raw(filename, [this, &has_searched_thumbtag, &has_detect_producer, &format, &task_flag, &out](GCodeReader& reader, const char* begin, const char* end) {
        begin = skip_whitespaces(begin, end);
        if (begin != end && *begin == ';') {
            // Comment.
            begin = skip_whitespaces(++begin, end);
            ...
            if (m_producer  == EProducer::AnkerMake || m_producer == EProducer::AnkerStudio || m_producer == EProducer::PrusaSlicer || m_producer == EProducer::Cura) {
                ...
                if (begin != end && detect_thumbnail_end(std::string_view(begin, end - begin), format, has_searched_thumbtag)) {
                    task_flag = task_flag | (1 << 0);
                    has_searched_thumbtag = false;
                    return;
                }
                   
                if (begin != end && has_searched_thumbtag) {
                ...
                //don't detect target producer quit parse
                m_parser.quit_parsing();
            }
...
/* std::cout << line.raw() << std::endl; */
    ...
    //friva,get ai cmd info
    //if (!m_result.m_pic_infos.empty()) {
    //    auto& pif = m_result.m_pic_infos.back();
    //    pif.X = m_end_position[0];
    //    pif.Y = m_end_position[1];
    //    pif.Z = m_end_position[2];
    //    pif.E = m_end_position[3];
    //    pif.F = m_feedrate;
    //}
    m_g1_line_pos.push_back(m_g1_line_id);
}
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::AnkerStudio) {
        // height tag
    ...
    if (comment == reserved_tag(ETags::Layer_Change)) {
        ++m_layer_id;
        if (m_layer_id > 1) {
            m_result.m_pic_infos.push_back(Slic3r::GCodeProcessorResult::picPosInfo{ m_layer_id-1,m_line_id,m_end_position[0], m_end_position[1], m_end_position[2],m_end_position[3],m_feedrate });
        }
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::AnkerStudio: { return process_ankerstudio_tags(comment); }
    case EProducer::AnkerMake:
...
bool GCodeProcessor::process_ankerstudio_tags(const std::string_view comment)
{
    return false;
}
...
void GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line, bool clockwise)
{
    ...
    // reference:
    // Anker-Firmware\Firmware\motion_control.cpp - mc_arc()
...
void GCodeProcessor::process_G92(const GCodeReader::GCodeLine& line)
{
    ...
    if (!any_found && !line.has_unknown_axis()) {
        // The G92 may be called for axes that AnkerStudio does not recognize, for example see GH issue #3510, 
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        if (static_cast<PrintEstimatedStatistics::ETimeMode>(i) == PrintEstimatedStatistics::ETimeMode::Normal ||
            ...
            if (line.has_value('S', value)) {
                ...
                // It is also generated by AnkerStudio to control acceleration per extrusion type
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            ...
            if (m_extruder_id != id) {
                if (((m_producer == EProducer::AnkerStudio || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) && id >= m_result.extruders_count) ||
                    ((m_producer != EProducer::AnkerStudio && m_producer != EProducer::Slic3rPE && m_producer != EProducer::Slic3r) && id >= m_result.extruder_colors.size()))
...
void GCodeProcessor::post_process()
{
    ...
    public:
        ...
        void synchronize_moves(GCodeProcessorResult& result) const {
            ...
            //sync v6 info && aipic info
            {
                for (std::vector<size_t>& v6info : result.m_v6_info.toolChangePos) {
                    for (size_t& pos : v6info) {
                        pos = m_gcode_lines_map[pos-1].second;
                    }
                }
                for (auto &picInfo : result.m_pic_infos) {
                    picInfo.line_id = m_gcode_lines_map[picInfo.line_id-1].second;
                }
...
bool Slic3r::GCodeProcessor::search_for_thumbnail_target(std::string_view line_str, const Vec2d& encode_sizes, std::string& format_str ,int task_flag)
{
    //if thumbnail has finshed then return  ,thumbnail task was set to the first bit (0) 
    if (task_flag & (1 << 0)) {
        return false;
    }

    double w = encode_sizes.x();
    double h = encode_sizes.y();
...
/*        std::string thumbhead = "thumbnail_";
    ...
    if (format_str != "") {
        //ankermake format always keep "thumbnail begin 256 256"
        search_string = (boost::format("thumbnail_%s begin %dx%d") % format_str % w % h).str();
    }
...
bool Slic3r::GCodeProcessor::detect_thumbnail_end(std::string_view line_str, const std::string& format_str, bool has_searched_tag)
{
    if (!has_searched_tag)
        return false;

    std::string search_string = (boost::format("thumbnail_%s end") % format_str ).str();
    if (format_str == "") {
         search_string = "thumbnail end";
    }
    const size_t pos = line_str.find(search_string);
    if (pos != line_str.npos) {
...
bool Slic3r::GCodeProcessor::search_for_speed_target(std::string_view line_str, std::string& speed)
{
    std::string search_string = "MAXSPEED:";
    if (m_producer == EProducer::AnkerMake) {
        const size_t pos  = line_str.find(search_string);
        if (pos != line_str.npos) {
            const std::string_view MAXSPEED = line_str.substr(pos + search_string.length());
            int maxSpeed;
            auto result = std::from_chars(MAXSPEED.data(), MAXSPEED.data() + MAXSPEED.size(), maxSpeed);
            speed = std::to_string(maxSpeed / 60);
            return true;
        }
        return false;
    }
    ...
    if (m_producer == EProducer::AnkerStudio) {
        search_string = "max_print_speed =";
        const size_t pos = line_str.find(search_string);
        if (pos != line_str.npos) {
            speed = line_str.substr(pos + search_string.length());
            return true;
        }
        return false;
    }
...
bool Slic3r::GCodeProcessor::search_for_filament_cost_target(std::string_view line_str, std::string& filament_cost)
{
    ...
    if (m_producer == EProducer::AnkerStudio) {
        search_string = "total filament used [g] =";
        const size_t pos = line_str.find(search_string);
        if (pos != line_str.npos) {
            filament_cost = line_str.substr(pos + search_string.length());
            filament_cost += "g";
...
bool Slic3r::GCodeProcessor::search_for_print_time_target(std::string_view line_str, float& print_time)
{
